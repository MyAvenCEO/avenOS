...
impl aven_db::CapabilityResolver for ServerApplyGate {
    fn may_sync(
        ...
        // Blind relay: store & forward ciphertext for everyone. The relay cannot evaluate a
        // SAFE's per-identity sync/replicate cap (those chains are sealed under each SAFE's
        // DEK), so OUTBOUND forwarding stays permissive — the **sync cap** is enforced on the
        // devices that hold it (a device only ships to / accepts from the relay what its caps
        // allow), and content stays E2E ciphertext the relay can't read. Authenticity is still
        // enforced on apply below (every owner-bearing row must carry a valid owner-binding).
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Err(e) = engine.set_resolver(std::sync::Arc::new(apply_gate)) {
        // (apply_gate enforces A3 binding authenticity; may_sync stays permissive — blind relay)
