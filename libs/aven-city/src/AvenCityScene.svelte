...
$effect(() => {
	...
	// Narrow once for the closures below — the effect re-runs when hexRadius changes.
	...
	function onPointerMove(e: PointerEvent) {
		...
		for (const plot of plots) {
			if (isInsideHex(x, y, plot.q, plot.r, radius)) {
				previewCoord = null
				return
			}
		}

		previewCoord = nearestExpansionCandidate(x, y, radius, occupiedKeys)
	}
