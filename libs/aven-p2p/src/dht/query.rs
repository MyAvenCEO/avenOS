...
/// A request produced by the query engine for the [`super::rpc::DhtNode`] to send.
#[derive(Debug)]
...
///
/// Driven externally by [`super::rpc::DhtNode`]:
...
mod tests {
    ...
    fn test_add_from_table_populates_pending() {
        ...
        let mut nid = [0u8; 32];
        nid[0] = 0x42;
        table.add(super::routing_table::Node {
            id: nid,
            host: "127.0.0.1".into(),
            port: 8888,
            token: None,
