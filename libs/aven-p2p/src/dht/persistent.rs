...
impl Persistent {
    ...
    pub fn on_lookup(&mut self, req: &IncomingHyperRequest) -> HandlerReply {
        ...
        let mut peers: Vec<HyperPeer> = self
            ...
            .into_iter()
            .filter_map(|bytes| {
                super::hyperdht_messages::decode_hyper_peer_from_bytes(&bytes).ok()
            })
            .collect();
        ...
        if peers.len() < MAX_RECORDS_PER_LOOKUP {
            if let Some(entry) = self.router.get(&key) {
                if let Ok(peer) =
                    super::hyperdht_messages::decode_hyper_peer_from_bytes(&entry.record)
                {
                    peers.push(peer);
                }
            }
    ...
    pub fn on_announce(
        ...
        if msg.peer.is_none() {
            if let Some(refresh) = &msg.refresh {
                ...
                if let Some(stored) = self.refreshes.get(&refresh_key) {
                    msg.peer = super::hyperdht_messages::decode_hyper_peer_from_bytes(&stored).ok();
                }
            }
        }
        ...
        if announce_self {
            ...
            if msg.bump != 0 {
                let mut bump_state = super::compact_encoding::State::new();
                super::compact_encoding::preencode_uint(&mut bump_state, msg.bump);
                bump_state.alloc();
                super::compact_encoding::encode_uint(&mut bump_state, msg.bump);
...
mod tests {
    ...
    fn on_lookup_no_target_is_silent() {
        ...
        let req = IncomingHyperRequest {
            command: super::hyperdht_messages::LOOKUP,
            target: None,
            token: None,
            value: None,
            from: dummy_peer(),
    ...
    fn immutable_put_get_roundtrip() {
        ...
        let put_req = IncomingHyperRequest {
            command: super::hyperdht_messages::IMMUTABLE_PUT,
            target: Some(target),
        ...
        let get_req = IncomingHyperRequest {
            command: super::hyperdht_messages::IMMUTABLE_GET,
            target: Some(target),
            token: None,
            value: None,
            from: dummy_peer(),
    ...
    fn mutable_put_get_roundtrip() {
        ...
        let sig = super::crypto::sign_detached(&signable, &sk);

        let put_msg = super::hyperdht_messages::MutablePutRequest {
            public_key: pk,
            seq,
            value: value.clone(),
            signature: sig,
        ...
        let put_bytes =
            super::hyperdht_messages::encode_mutable_put_request_to_bytes(&put_msg).unwrap();

        let put_req = IncomingHyperRequest {
            command: super::hyperdht_messages::MUTABLE_PUT,
            target: Some(target),
            token: Some([0u8; 32]),
        ...
        let mut seq_state = super::compact_encoding::State::new();
        super::compact_encoding::preencode_uint(&mut seq_state, 0u64);
        seq_state.alloc();
        super::compact_encoding::encode_uint(&mut seq_state, 0u64);

        let get_req = IncomingHyperRequest {
            command: super::hyperdht_messages::MUTABLE_GET,
            target: Some(target),
            token: None,
        ...
        if let HandlerReply::Value(Some(v)) = reply {
            let decoded =
                super::hyperdht_messages::decode_mutable_get_response_from_bytes(&v).unwrap();
    ...
    fn on_announce_invalid_signature_is_silent() {
        ...
        let peer = super::hyperdht_messages::HyperPeer {
            public_key: pk,
            relay_addresses: vec![],
        };

        // Use a bad signature.
        let ann = super::hyperdht_messages::AnnounceMessage {
            peer: Some(peer),
            refresh: None,
        ...
        let ann_bytes =
            super::hyperdht_messages::encode_announce_to_bytes(&ann).unwrap();

        let req = IncomingHyperRequest {
            command: super::hyperdht_messages::ANNOUNCE,
            target: Some(target),
            token: Some([0u8; 32]),
