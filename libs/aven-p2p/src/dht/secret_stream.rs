...
use super::crypto;
use super::noise::{self, Keypair, NoiseError};
use super::secretstream::{self, ABYTES, HEADERBYTES, KEYBYTES};
...
// ── Split halves (AvenOS patch) ──────────────────────────────────────────────
//
// AvenOS-local addition: split a `SecretStream<T>` into independent read and
// write halves so a reader task and a writer task can use the same encrypted
// connection concurrently without sharing a `&mut self`.
//
// This is necessary because `SecretStream::read` suspends the task while
// waiting for the next inbound frame; if read+write share `&mut self` (or a
// `Mutex<SecretStream>`), the writer can never make progress while the reader
// is blocked. The standard Tokio escape hatch — `tokio::select!` between read
// and write futures — is unsafe here because `SecretStream::read` calls
// `read_exact` internally, which is NOT cancellation-safe: dropping the
// future mid-frame leaves partially-consumed bytes that the next read can
// never recover, then `secretstream::Pull::next` fails authentication and the
// link dies with `Decrypt(DecryptionFailed)`.
//
// Splitting is safe because the encrypt and decrypt state machines are
// completely independent (separate libsodium counters) — only the underlying
// raw transport needs to be split, which `tokio::io::split` handles cleanly.

/// Read half of a split [`SecretStream`].
///
/// Returned by [`SecretStream::into_split`]. Owns the decrypt state and the
/// read half of the underlying transport; can be moved to a dedicated reader
/// task that runs concurrently with the writer half.
pub struct SecretStreamReader<R> {
    raw: R,
    decrypt: secretstream::Pull,
    remote_public_key: [u8; 32],
    handshake_hash: [u8; 64],
...
/// Write half of a split [`SecretStream`].
///
/// Returned by [`SecretStream::into_split`]. Owns the encrypt state and the
/// write half of the underlying transport; can be moved to a dedicated writer
/// task that runs concurrently with the reader half.
pub struct SecretStreamWriter<W> {
    raw: W,
    encrypt: secretstream::Push,
    remote_public_key: [u8; 32],
    handshake_hash: [u8; 64],
...
impl<R: AsyncRead + Unpin> SecretStreamReader<R> {
    /// Read and decrypt the next framed message. Mirrors [`SecretStream::read`].
    pub async fn read(&mut self) -> Result<Option<Vec<u8>>, SecretStreamError> {
        loop {
            let msg = match read_frame(&mut self.raw).await? {
                Some(m) => m,
            ...
            // Drop empty keepalive messages (match Node.js behaviour).
            if plaintext.is_empty() {
                continue;
            }
    ...
    /// Remote peer's static Ed25519 public key (same as the un-split stream).
    pub fn remote_public_key(&self) -> &[u8; 32] {
        &self.remote_public_key
    }
...
impl<W: AsyncWrite + Unpin> SecretStreamWriter<W> {
    /// Encrypt and send `data` as a single framed message. Mirrors [`SecretStream::write`].
    pub async fn write(&mut self, data: &[u8]) -> Result<(), SecretStreamError> {
        let encrypted = self.encrypt.next(data);
        debug_assert_eq!(encrypted.len(), data.len() + ABYTES);
        write_frame(&mut self.raw, &encrypted).await
...
impl<T: AsyncRead + AsyncWrite + Unpin + Send + 'static> SecretStream<T> {
    /// Split this `SecretStream` into independent read and write halves.
    ///
    /// The reader and writer can be moved to separate tasks and used
    /// concurrently. This is the canonical way to use a `SecretStream` from
    /// a Tokio bridge that needs full-duplex sync traffic — sharing the
    /// stream behind a mutex causes the reader to deadlock the writer (the
    /// reader holds the lock across `await` while waiting for inbound bytes),
    /// and `tokio::select!` cannot be used safely because `SecretStream::read`
    /// is not cancellation-safe.
    pub fn into_split(
        self,
    ) -> (
        SecretStreamReader<tokio::io::ReadHalf<T>>,
        SecretStreamWriter<tokio::io::WriteHalf<T>>,
    ) {
        let SecretStream {
            raw,
            encrypt,
        ...
        let (raw_r, raw_w) = tokio::io::split(raw);
        (
            SecretStreamReader {
                raw: raw_r,
                decrypt,
...
impl<T: AsyncRead + AsyncWrite + Unpin + Send + 'static> super::protomux::FramedStream
    for SecretStream<T>
{
