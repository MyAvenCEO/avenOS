...
For the native app, build from the repo root with **`bun run fetch:webcm`** / the app dev scripts; the sandbox watcher is started by **`bun run dev:app`** (see `app/` and `scripts/`).
...
- **`@avenos/aven-vibes`** (`libs/aven-vibes`) — Catalog of built single-file HTML bundles + demo `toolArguments` for each mini-app.
