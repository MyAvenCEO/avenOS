...
async function handleFetch(req: Request): Promise<Response> {
	if (!(await SANDBOX_FILE.exists())) {
		return new Response(
			'Run `bun run build` (in @avenos/aven-vibe-sandbox) first — missing sandbox/dist/sandbox.html.',
			{ status: 503 }
		)
	}
	...
	if (url.pathname === '/ext-apps.js') {
		if (!(await EXT_APPS_FILE.exists())) {
			return new Response(
				'Run `bun run build:ext-apps` (in @avenos/aven-vibe-sandbox) — missing sandbox/dist/ext-apps.js.',
				{ status: 503 }
			)
		}
