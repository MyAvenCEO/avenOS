...
export const STATEMENT_VALIDATOR_MANIFEST = manifest(
	'statement-validator',
	'Statement validator',
...
export function createStatementValidatorActor(): Actor {
	return new Actor(structuredClone(STATEMENT_VALIDATOR_MANIFEST), {
			...
			try {
				...
				const validation = {
					...
					coverageBps: Math.floor(
						(outcomes.filter((outcome) => outcome !== 'UNKNOWN').length * 10_000) / outcomes.length
					),
					checks: [
						{
