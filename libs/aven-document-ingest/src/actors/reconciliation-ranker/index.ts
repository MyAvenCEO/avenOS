...
export const RECONCILIATION_RANKER_MANIFEST = manifest(
...
export function createReconciliationRankerActor(): Actor {
	return new Actor(structuredClone(RECONCILIATION_RANKER_MANIFEST), {
		reconciliation_rank_invoice_transactions: (payload) => {
			try {
				...
				const transactions = payload.transactions.map(
					(value) => object(value, 'reconciliation transaction') as unknown as StatementTransaction
				)
