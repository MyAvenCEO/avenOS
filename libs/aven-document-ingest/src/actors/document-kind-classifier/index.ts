...
export const DOCUMENT_KIND_CLASSIFIER_MANIFEST = manifest(
...
export function createDocumentKindClassifierActor(
	...
	return new Actor(structuredClone(DOCUMENT_KIND_CLASSIFIER_MANIFEST), {
		document_classify_kind: async (payload) => {
			try {
				...
				const rawKind = stringValue(structured.resolvedKind ?? structured.rawKind, 'document kind')
				const family = [
					'invoice',
					'credit-note',
					'receipt',
