...
live(
	...
	async () => {
		...
		const source = documentSource(
			textPdf(
				Array.from({ length: 2 }, (_, page) => [
					...
					'DOCUMENT TOTALS (all 2 pages): Net EUR 6.00. VAT EUR 0.00. Gross EUR 6.00. Amount due EUR 6.00.',
					'All items are VAT exempt. No payments received. No discounts or additional charges.',
					...
					...Array.from(
						{ length: 3 },
						(_, i) => `Item ${page * 3 + i + 1} | 1 | 1.00 | 1.00 | 0.00 | 1.00`
					)
				])
			),
		...
		await writeFile(
			...
			JSON.stringify({ result, candidate, details, receipts }, null, 2)
		)
