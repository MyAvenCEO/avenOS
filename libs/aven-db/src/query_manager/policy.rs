...
// Catalogue wire types only — ReBAC evaluation removed (AvenOS uses Biscuit ACC in app).

/// Legacy complex clause marker (graph evaluation removed).
#[derive(Debug, Clone)]
...
pub fn evaluate_simple_parts_with_row_id(
    _expr: &PolicyExpr,
    _content: &[u8],
    _provenance: &crate::metadata::RowProvenance,
    _descriptor: &super::types::RowDescriptor,
