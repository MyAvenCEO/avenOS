...
use crate::transport::TickMode;

const PAIR_CODE_ALPHABET: &[u8] = b"23456789ABCDEFGHJKLMNPQRSTUVWXYZ";

/// During relay-first invite, clear stale `SwarmConnecting` / `Discovering` rows after this age.
pub use crate::transport::STALE_SWARM_CONNECTING_MS as PAIRING_SWARM_STALE_MS;

/// During relay-first invite, suppress duplicate relay link churn while mux is live.
pub const PAIRING_TRANSPORT_GUARD_SECS: u64 = 90;
...
impl PairingState {
	...
	pub fn accepts_transport_tick(&self, mode: TickMode) -> bool {
		match mode {
			TickMode::Pairing | TickMode::Reset => true,
			_ => matches!(
				self.phase,
