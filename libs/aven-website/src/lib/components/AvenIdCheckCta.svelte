...
<form onsubmit={submit} class={wrapperClass} aria-label="AvenCEO‑Name zur Warteliste">
	...
	<p class="mt-3 max-w-2xl text-[14px] leading-snug text-foreground/68 sm:text-[15px]">
		...
		und passende Mail unter <strong class="font-medium text-foreground/82">dein-name@aven.ceo</strong>.
	</p>
	<div class="mt-6 flex flex-col gap-2.5 sm:flex-row sm:items-stretch">
		<label class="flex min-h-12 flex-1 items-center gap-2 rounded-full border border-border/60 bg-white/85 px-4 ring-1 ring-black/4">
			<input
				bind:value={name}
				type="text"
				name="aven-name"
