//#region src/lib/intent-mock/beam-avatar.ts
/**
...
function getUnit(number, range, index) {
	...
	if (index !== void 0 && getDigit(number, index) % 2 === 0) return -value;
	return value;
}
...
function pickColor(number, colors, len) {
...
function paletteFromCommaString(csv) {
	const parts = csv.split(",").map((p) => p.trim()).filter(Boolean).map((p) => normalizeHex(p));
	return parts.length > 0 ? parts : DEFAULT_PALETTE.map(normalizeHex);
}
/** Stable SVG string for {@html} (fixed mock inputs only). */
function beamAvatarSvg(name, colors, sizePx, domSafeMaskId) {
	const t = beamData(name, colors.length > 0 ? colors : DEFAULT_PALETTE);
	const mouthPath = t.isMouthOpen ? `<path d="M15 ${19 + t.mouthSpread}c2 1 4 1 6 0" stroke="${t.faceColor}" fill="none" stroke-linecap="round"/>` : `<path d="M13,${19 + t.mouthSpread} a1,0.75 0 0,0 10,0" fill="${t.faceColor}"/>`;
...
//#endregion
export { paletteFromCommaString as n, beamAvatarSvg as t };
