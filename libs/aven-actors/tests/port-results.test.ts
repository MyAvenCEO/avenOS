...
import {
	buildExecutionReceiptRecord,
	buildPortResultRecords,
	type CapabilitySlot,
	checkNamedPortResults,
	type NamedPortResult,
	PortResultError,
	type RuntimeArtifact,
	readPortResultMembership,
	resourceId
} from '../src'

const schema = resourceId({
	authority: 'ceo.aven',
	kind: 'schema',
	namespace: 'fixture',
	name: 'report',
...
const many: CapabilitySlot = {
	name: 'members',
	predicate: 'fixture.member(X)',
	schema,
	role: 'member',
	cardinality: 'many'
}
...
describe('named Actor port results', () => {
	test('normalizes shuffled return order and records empty optional and many ports', () => {
		const checked = checkNamedPortResults(
			[one, optional, many],
			[
				result(many, []),
				result(optional, []),
				result(one, [member(one, 'first', { text: 'same' })])
			]
		)
	...
	test('equal bytes do not collapse separate ordered occurrences', () => {
		const checked = checkNamedPortResults(
			[many],
			[result(many, [member(many, 'left', 'same bytes'), member(many, 'right', 'same bytes')])]
		)
		expect(checked.newMembers.map((entry) => entry.memberKey)).toEqual(['left', 'right'])
		expect(checked.newMembers).toHaveLength(2)
		expect(buildPortResultRecords('invocation-exact', checked).portResults[0]?.references).toEqual([
			{ role: 'member', localKey: 'domain-0' },
	...
	test('rejects undeclared, omitted, duplicate and forged member contracts', () => {
		...
		expect(() =>
			checkNamedPortResults(
				[many],
				[result(many, [member(many, 'same', 1), member(many, 'same', 2)])]
			)
			...
			checkNamedPortResults(
				...
				[
					result(one, [
						{
							...member(one, 'first', 1),
							draft: { ...member(one, 'first', 1).draft, predicate: 'fixture.private(X)' }
						}
					])
				]
			)
	...
	test('reuses only exact previously admitted committed occurrences', () => {
		...
		expect(checkNamedPortResults([one], [port], new Set(['exact-id'])).reusedMembers).toMatchObject(
			[{ memberKey: 'cached', artifact: { artifactId: 'exact-id' } }]
		)
		expect(
			buildPortResultRecords(
				'invocation-exact',
				checkNamedPortResults([one], [port], new Set(['exact-id']))
			).portResults[0]?.references
	...
	test('zero-output completion has no invented domain result', () => {
		...
		const input: RuntimeArtifact = {
			artifactId: 'source-exact',
			predicate: one.predicate,
			schema,
			typeKey: 'fixture.summary',
			schemaVersion: 1,
		...
		expect(
			buildExecutionReceiptRecord(
				'invocation-exact',
				'os.aven:capability:fixture:notify@1',
				'registered-instance',
				'completed',
				[{ slot: 'source', role: 'input', artifact: input }]
			)
		).toMatchObject({
	...
	test('read-side binding ignores row order but rejects lost, duplicate and redacted references', () => {
		const payload = {
			invocationId: 'exact-call',
			port: 'members',
			cardinality: 'many' as const,
			members: [
				{ memberKey: 'left', referenceOrdinal: 0 },
				{ memberKey: 'right', referenceOrdinal: 1 }
			]
		}
		const refs = [
			{ role: 'member', ordinal: 0, artifactId: 'origin-left' },
		...
		expect(() => readPortResultMembership('exact-call', payload, refs.slice(0, 1), many)).toThrow(
			'membership and references disagree'
		)
		expect(readPortResultMembership('exact-call', payload, [refs[1], refs[0]], many)).toEqual([
			{ memberKey: 'left', artifactId: 'origin-left' },
			{ memberKey: 'right', artifactId: 'origin-right' }
		])
		expect(() =>
			readPortResultMembership('exact-call', payload, [refs[0], { ...refs[1], ordinal: 0 }], many)
		).toThrow('ordinals are invalid')
		expect(() =>
			readPortResultMembership(
				'exact-call',
				payload,
				[refs[0], { ...refs[1], artifactId: null }],
				many
			)
		...
		expect(() => readPortResultMembership('different-call', payload, refs, many)).toThrow(
			'differs from its declared port'
		)
	})
})
