	...
	methods: [
		{
			...
			produces: ['fixture.value(X)'],
			outputSlots: [
				{
					name: 'value',
					predicate: 'fixture.value(X)',
					schema: 'ceo.aven:schema:studio:value@1',
					role: 'value',
...
const descriptor = (): ActorInstallationDescriptor => ({
	packageId: 'ceo.aven:package:studio:fixture@1',
	manifest: structuredClone(manifest),
	placements: ['server'],
	schemas: [
		{
			schema: 'ceo.aven:schema:studio:value@1',
			typeKey: 'fixture.value',
			typeVersion: 1,
...
describe('trusted Actor installation descriptors', () => {
	...
	test('requires a new version when projector, implementation, or owning package changes', async () => {
		...
		for (const change of [
			(d: ActorInstallationDescriptor) => {
				d.schemas[0]!.projectorDigest = 'b'.repeat(64)
			},
			(d: ActorInstallationDescriptor) => {
				d.procedures[0]!.implementationDigest = 'b'.repeat(64)
			},
			(d: ActorInstallationDescriptor) => {
				d.packageId = 'another-package'
			}
		]) {
			const changed = descriptor()
			change(changed)
			await expect(installs.install(changed)).rejects.toThrow(
				'ACTOR_INSTALLATION_IDENTITY_CONFLICT'
			)
		}
	})
	...
	test('rejects foreign procedure IDs and invalid projector digests', async () => {
		...
		await expect(installs.install(badProjector)).rejects.toThrow(
			'ACTOR_INSTALLATION_SCHEMA_INVALID'
		)
	...
	test('accounts for proof, zero-output view, and incomplete methods without running them', async () => {
		...
		d.manifest.methods.push({
			name: 'unknown',
			description: 'Needs contracts',
			parameters: { type: 'object' },
			requires: ['fixture.value(X)'],
