...
import { type ActorDefinition, definitionFromManifest, type ExecutionEnvironment } from './registry'
...
export class TrustedActorInstallations {
	...
	list(): TrustedActorInstallation[] {
		return [...this.#byRef.values()].sort((a, b) =>
			a.definition.ref.localeCompare(b.definition.ref)
		)
	}
	...
	inventory(): ActorInstallationInventoryEntry[] {
		return this.list()
			.flatMap(({ descriptor, definition, digest }) =>
				definition.capabilities.map((capability) => {
					const method = descriptor.manifest.methods.find(
						(item) => item.name === capability.method
					)!
					const reasons = methodInventoryIssues(
						descriptor,
						capability.id,
						method,
						capability.requires.length,
						capability.produces.length
					)
					return {
	...
	async digest(): Promise<string> {
		return sha256(
			this.list()
				.map((item) => `${item.definition.ref}\0${item.digest}`)
				.join('\n')
		)
	}
}
...
function validateDescriptor(
	...
	if (
		!exactKeys(descriptor, ['packageId', 'manifest', 'placements', 'schemas', 'procedures']) ||
		!descriptor.packageId ||
		descriptor.packageId.length > 256 ||
		...
		!Array.isArray(descriptor.schemas) ||
		!Array.isArray(descriptor.procedures)
	)
		throw new Error('ACTOR_INSTALLATION_INVALID')
	if (
		new Set(definition.capabilities.map((capability) => capability.id)).size !==
	...
	for (const binding of descriptor.schemas) {
		if (
			!exactKeys(binding, ['schema', 'typeKey', 'typeVersion', 'projectorDigest']) ||
			typeof binding.schema !== 'string' ||
			!binding.schema ||
			binding.schema.length > 160 ||
			!typeKey(binding.typeKey) ||
			!Number.isInteger(binding.typeVersion) ||
			binding.typeVersion < 1 ||
			binding.typeVersion > 10_000 ||
			!hexDigest(binding.projectorDigest) ||
			schemas.has(binding.schema)
		)
	...
	for (const binding of descriptor.procedures) {
		if (
			...
			!known.has(binding.capabilityId) ||
			procedures.has(binding.capabilityId) ||
			!binding.procedureKey ||
			!binding.procedureVersion ||
			!hexDigest(binding.implementationDigest) ||
			!hexDigest(binding.evidenceRuleDigest) ||
			!Array.isArray(binding.requiredFeatures) ||
			!binding.requiredFeatures.every(
...
function methodInventoryIssues(
	...
	if (
		!descriptor.procedures.some((binding) => binding.capabilityId === capabilityId) &&
		method.mode !== 'view'
	)
		issues.push('PROCEDURE_UNDECLARED')
	...
	for (const slot of publicSlots) {
		if (!slot.schema || !slot.role) issues.push('PORT_CONTRACT_INCOMPLETE')
			...
			!slot.sensitive &&
			!descriptor.schemas.some((binding) => binding.schema === slot.schema)
		)
