...
function refinedBranchPorts(
	...
	const key =
		binding.kind === 'input'
			? `input:${binding.port}`
			: `step:${binding.stepId}:${binding.port}`
...
export function studioSkillChildReferences(definition: StudioSkillV2): string[] {
	return [...new Set(allSteps(definition.steps).flatMap((step) =>
		step.kind === 'skill' ? [step.skillArtifactId] :
			step.kind === 'forEach' ? [step.childSkillArtifactId] : []))]
}
