	...
	return value.map((raw, index): StudioSkillStep => {
		...
		if (item.kind === 'invoke') {
			...
			return {
				...base,
				kind: 'invoke',
				capabilityId: shortText(item.capabilityId, at, 256),
		...
		for (const [index, step] of body.entries()) {
			...
			if (step.kind === 'when') {
				...
				for (const [branchName, branch] of Object.entries(step.branches)) {
					...
					const branchState = walk(branch.steps, `${branchPath}.steps`, state)
					if (
...
export function validateStudioSkillCapabilities(
	...
	const known = new Map<string, StudioSkillPort>()
	for (const [name, value] of Object.entries(definition.inputs)) known.set(`input:${name}`, value)
	for (const [index, step] of definition.steps.entries()) {
		...
		for (const slot of capability.inputSlots ?? []) {
			if (slot.sensitive && slot.name in step.inputs)
				issues.push({
					path: `steps.${index}.inputs.${slot.name}`,
					code: 'PROTECTED_INPUT',
					message: 'Protected input must be resolved by the trusted host.'
				})
			else if (!slot.sensitive && slot.cardinality === 'one' && !(slot.name in step.inputs))
				issues.push({
					path: `steps.${index}.inputs.${slot.name}`,
					code: 'MISSING_PORT',
		...
		for (const slot of capability.outputSlots ?? [])
			if (!(slot.name in step.outputs))
				issues.push({
					path: `steps.${index}.outputs.${slot.name}`,
					code: 'MISSING_PORT',
