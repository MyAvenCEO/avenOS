	...
	for (const step of steps) {
		...
		if (step.kind === 'skill') {
			...
			state.completions.set(
				`${prefix}.${step.id}`,
				terminalSteps(state.steps.slice(startedAt))
			)
			continue
		}
...
function compileInvocation(
	...
	if (!authorized || !target) throw new Error(`STUDIO_CAPABILITY_NOT_EXECUTABLE:${step.capabilityId}`)
