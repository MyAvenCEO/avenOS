...
function completeContract(
	...
	if (
		[...(method.inputSlots ?? []), ...(method.outputSlots ?? [])].some(
			(slot) => !validPortShape(slot)
		)
	)
		reasons.push('PORT_CONTRACT_INVALID')
	...
	if (!installation) reasons.push('INSTALLATION_UNAVAILABLE')
		...
		if (
			method.mode !== 'view' &&
			!installation.descriptor.procedures.some((binding) => binding.capabilityId === capability.id)
		)
