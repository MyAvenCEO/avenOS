...
export const PORT_RESULT_SCHEMA = resourceId({
	authority: 'os.aven',
	kind: 'schema',
	namespace: 'actors',
	name: 'port-result',
...
export const EXECUTION_RECEIPT_SCHEMA = resourceId({
	authority: 'os.aven',
	kind: 'schema',
	namespace: 'actors',
...
export const EXECUTION_RECEIPT_ARTIFACT_TYPE = {
	key: 'actors.execution-receipt',
	version: 1
} as const
...
export function checkNamedPortResults(
	...
	for (const slot of declared) {
		if (slots.has(slot.name))
			throw new PortResultError('DUPLICATE_DECLARED_PORT', 'Output port is declared twice.')
		if (!slot.schema || !slot.role || slot.sensitive)
			throw new PortResultError(
				'INCOMPLETE_PORT_CONTRACT',
				'Output port needs a public schema and role.'
			)
	...
	for (const result of returned) {
		...
		if (!slot)
			throw new PortResultError('UNDECLARED_PORT', 'Actor returned an undeclared output port.')
		if (seen.has(result.port))
			throw new PortResultError('DUPLICATE_PORT', 'Actor returned one port twice.')
		if (result.cardinality !== slot.cardinality || !Array.isArray(result.members))
			throw new PortResultError(
				'CARDINALITY_MISMATCH',
				'Actor result cardinality differs from its contract.'
			)
		if (
			(slot.cardinality === 'one' && result.members.length !== 1) ||
			(slot.cardinality === 'optional' && result.members.length > 1) ||
			(slot.cardinality === 'many' && result.members.length > 256)
		)
			throw new PortResultError(
				'CARDINALITY_MISMATCH',
				'Actor returned an invalid number of members.'
			)
		...
		for (const member of result.members) {
			if (!memberKeyPattern.test(member.memberKey) || keys.has(member.memberKey))
				throw new PortResultError(
					...
					'Member keys must be unique stable names.'
				)
			keys.add(member.memberKey)
			if (member.kind === 'new') {
				if (
					member.draft.slot !== slot.name ||
					member.draft.role !== slot.role ||
					member.draft.schema !== slot.schema ||
					member.draft.predicate !== slot.predicate
				)
					throw new PortResultError(
						'OUTPUT_CONTRACT_MISMATCH',
						'New member differs from its declared port.'
					)
				...
				if (!admittedReuseIds.has(member.artifact.artifactId))
					throw new PortResultError(
						'UNADMITTED_REUSE',
						'Reused occurrence was not admitted for this invocation.'
					)
				if (member.artifact.schema !== slot.schema || member.artifact.predicate !== slot.predicate)
					throw new PortResultError(
						'OUTPUT_CONTRACT_MISMATCH',
						'Reused member differs from its declared port.'
					)
	...
	if (seen.size !== slots.size)
		throw new PortResultError(
			'MISSING_PORT_RESULT',
			'Every declared output port needs a result, including empty ports.'
		)
	...
	for (const result of ports)
		for (const member of result.members)
			if (member.kind === 'new')
				newMembers.push({ port: result.port, memberKey: member.memberKey, draft: member.draft })
			else
				reusedMembers.push({
					port: result.port,
...
export function buildExecutionReceiptRecord(
	...
	if (
		...
		!implementationRef ||
		implementationRef.length > 256 ||
		inputs.length > 16
	)
		throw new PortResultError(
			'INVALID_RECEIPT',
			'Completion receipt has an invalid exact identity.'
		)
	return {
...
export function readPortResultMembership(
	...
	if (
		...
		payload.invocationId !== expectedInvocationId ||
		payload.port !== declared.name ||
		payload.cardinality !== declared.cardinality ||
		!Array.isArray(payload.members) ||
		payload.members.length > 256
	)
		throw new PortResultError(
			'PORT_RESULT_CONTRACT_MISMATCH',
			'Committed result differs from its declared port.'
		)
	if (
		(declared.cardinality === 'one' && payload.members.length !== 1) ||
		(declared.cardinality === 'optional' && payload.members.length > 1)
	)
		throw new PortResultError(
			'PORT_RESULT_CARDINALITY',
			'Committed result has an invalid member count.'
		)
	const members = references.filter((reference) => reference.role === 'member')
	if (members.length !== payload.members.length || references.length !== members.length)
		throw new PortResultError(
			'PORT_RESULT_REFERENCE_MISMATCH',
			'Result membership and references disagree.'
		)
	...
	for (const reference of members) {
		if (
			...
			reference.ordinal < 0 ||
			reference.ordinal >= members.length ||
			byOrdinal.has(reference.ordinal)
		)
			throw new PortResultError(
				'PORT_RESULT_REFERENCE_MISMATCH',
				'Result reference ordinals are invalid.'
			)
	...
	return payload.members.map((entry, index) => {
		...
		if (
			!reference ||
			entry.referenceOrdinal !== index ||
			!memberKeyPattern.test(entry.memberKey) ||
			keys.has(entry.memberKey)
		)
			throw new PortResultError(
				'PORT_RESULT_REFERENCE_MISMATCH',
				'Result reference order is invalid.'
			)
		keys.add(entry.memberKey)
		if (!reference.artifactId)
			throw new PortResultError(
				'PORT_RESULT_MEMBER_UNAVAILABLE',
				'A result member is unavailable under current authority.'
			)
