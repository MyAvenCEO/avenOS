...
/** The line a user hands to Claude Code's built-in `/goal` loop. */
const goalCommand = $derived(item.goal ? `/goal ${item.goal}` : '')

let copied = $state(false)
let copyTimer: ReturnType<typeof setTimeout> | undefined

async function copyGoal(): Promise<void> {
	if (!goalCommand) return
	try {
		await navigator.clipboard.writeText(goalCommand)
		copied = true
		...
		/* clipboard unavailable — leave the text on screen to copy by hand */
	}
}
</script>
...
>
	<div class="min-h-0 flex-1 overflow-y-auto px-4 pt-6 pb-28 sm:px-6">
		...
		>
			...
			{#if item.goal}
				<section class="mb-8 rounded-2xl border border-primary/30 bg-primary/5 p-4 sm:p-5">
					<div class="flex items-center justify-between gap-3">
						<p class="text-[10px] font-bold tracking-widest text-primary uppercase opacity-80">
					...
					<p class="mt-2 font-mono text-sm leading-relaxed text-foreground">{item.goal}</p>
					<p class="mt-3 text-[11px] leading-relaxed text-muted-foreground">
						Hand off to Claude Code:
						<code class="rounded bg-muted/80 px-1 py-0.5">/board-goal {item.id}</code>
						<span class="opacity-60">— or</span>
						<code class="rounded bg-muted/80 px-1 py-0.5">/goal</code>
