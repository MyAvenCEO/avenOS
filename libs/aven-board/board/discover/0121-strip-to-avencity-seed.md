...
- [ ] Biometric reference is parked and explained — proven by `find ARCHIVE/tauri-plugin-biometric -name '*.rs' | wc -l` being non-zero and `rg -c 'network seed' ARCHIVE/tauri-plugin-biometric/README.md`.
- [ ] Passkey reference documents the domain contract — proven by `rg -i 'apple-app-site-association|webcredentials|RP ID' ARCHIVE/tauri-plugin-passkey/README.md` matching all three.
