...
- [ ] `bun run check` exits 0 (svelte-check + tsc across touched libs). — `bun run check`
- [ ] `bun run lint` exits 0 (biome). — `bun run lint`
- [ ] New realtime-voice client + proxy broker routes compile; unit tests green
      (session gate/401, model-id mapping, PCM framing, sentence chunking). — `bun test`
- [ ] Voice-mode store defaults to `realtime`, toggles to `on-device`, persists —
      asserted by test. — `bun test app/tests/voice-mode-store.test.ts`
- [ ] AI settings tab renders the switcher (component/store test). — `bun test`
- [ ] On-device Parakeet→LFM2→MOSS path unchanged and its existing tests still pass. —
      `bun test app/tests/transcribe.test.ts app/tests/transcribe-progress.test.ts`
- [ ] Only the files listed above changed. — `git status --porcelain`
