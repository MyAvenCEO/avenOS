...
- [ ] `importance` exists — `grep -n "importance" libs/aven-brain/src/brain.rs libs/aven-brain/src/schema.rs` hits.
- [ ] `cargo test -p aven-brain memory_tools -- --nocapture` exits 0 and prints: (a) importance ordering, (b) forget removes from recall, (c) attest raises veracity weight, (d) explicit link traversed.
- [ ] `cargo test -p aven-brain` exits 0 (28+ tests; no regression).
- [ ] Graph/fact recall voice present — `grep -n "graph\|fact" libs/aven-brain/src/brain.rs` shows it inside `search_traced`/recall.
- [ ] Memory tools wired — `grep -n "memory_remember\|memory_recall\|MEMORY_TOOL_DEFS" app/src/lib/llm/tools.ts app/src/lib/llm/memory-tools.ts` hits and they're in `CLOUD_TOOLS`.
- [ ] `memory_forget` is HITL-gated — `grep -n "memory_forget\|requestConfirm" app/src/lib/identities/identity-agent.svelte.ts` shows the gate.
