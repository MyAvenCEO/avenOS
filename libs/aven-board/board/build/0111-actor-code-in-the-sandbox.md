...
- `2026-07-03` — **Slice 2a: async caps SOLVED + green.** The crux (a real DB `ops` cap suspends the VM) is cracked: `actor-sandbox.ts` now uses the async context (`newAsyncContext` + `newAsyncifiedFunction` + `evalCodeAsync` + `executePendingJobs` + `getPromiseState` — NOT `resolvePromise`, which deadlocked). `actor-sandbox.test.ts` (7 pass) now exercises an **async** cap; fail-closed + fuel still hold. tsc green. **Remaining (slice 2b):** code-vs-engine runner dispatch + build real caps (ops→runOperation), port the todos vertical to `actor.code`, retire `vibe_logic`, chat/UI parity test.
