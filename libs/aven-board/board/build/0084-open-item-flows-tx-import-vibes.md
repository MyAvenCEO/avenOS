...
- `2026-06-29` — **Full e2e invoice flow built** (English). Three composite sub-skills + a HITL gate: **doc-ingest** (ingest→classify) → **capture** (extract→enrich) → **book** (match→post SKR04) → top **invoice** flow chains them via composites + a `review` node (`hitl: true`) accepting the e2e. Added `hitl` to the node model + a HITL badge in FlowNodeCard; `approval` resource label. Runs: `run-invoice-e2e` (4 composites, ends `waiting` at review) + `run-ingest-1`. Audit 0 issues; flatten(invoice)=7 leaves; 27 aven-skills tests; both tsc 0; app svelte-check clean; biome clean.

- `2026-06-29` — **RESET — ground-up skill-by-skill rebuild** (user). Removed ALL prior mock flows/skills (doc-ingest variants, rechnung/vertrag, bank-statement, open-item-match, tx-import, month-close, minecraft, outgoing-invoice) + their runs + the tx-import ingest config. Kept the engine (flow.ts schema/helpers/capability, FlowGraph/RunsView/StepVibe + vibe components). Rebuilt the test suite to exercise the engine on inline fixtures + the shipped skill. **Planned hierarchy** (nested composites, built one at a time): (1) **doc-ingest** = ingest(store→fs) + classify(LLM) ✅ DONE; (2) extract(LLM) + enrich(db create/update) → composite; (3) match(tx) + book(SKR04) → composite; (4) HITL review accepting the e2e invoice flow. Now: only doc-ingest ships. 23 aven-skills tests green; both libs tsc 0; app svelte-check only __APP_VERSION__; biome clean. Card moved review→build (active).

- `2026-06-29` — Standardised skill↔sub-skill wiring (review feedback): collapsed the two-flow split into ONE **Beleg-Ingest** flow where `classify` forwards **directly** to the per-type extractor sub-skills (rechnung / bank-statement / vertrag), then **shared** enrich → book-open after. Removed the separate `ingest` (Dokument-Ingest) flow + the »Beleg verbuchen« wrapper layer (classify was hidden inside it). Now uniform: classify → {Rechnung | Kontoauszug | Vertrag} composites → shared booking. Runs collapsed to one full doc-ingest walk (import→store→classify→extract→enrich→book-open). 9 flows, audit 0 issues, 31 tests green, app svelte-check clean.
