...
| **E4** | **Context manager**: `assemble_context` wiring + `context_traces` writes + fallback path. **Rewire the LFM2.5 toolset to brain tools exclusively**: replace the demo-era `navigate_views` + todo-CRUD tool types (`LLM_TOOLS`, `resolveAgentTurn`, the todo preamble) with brain tools — `remember`/`note` (writes a structured-note memory via the text pipeline), `search`/`recall`, `entity_card`. Old tool types deleted; artifact-action tools (todos etc.) return later as adapter-backed tools obeying the one-way-truth rule (tool writes artifact → brain ingests) | `identity-agent.svelte.ts`, `app/src/lib/llm/tools.ts` | reply still streams with brain off (fallback); trace row per human message; prompt contains L0/L1/WW/recall blocks under budget |
