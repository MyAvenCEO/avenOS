...
- [x] `libs/` holds exactly two packages — `ls libs` → **aven-board, aven-city** (was 17).
- [x] No dead references to either world remain in live code — `rg -il --glob '!**/*.md' 'abagana|alberobello|better-auth|aven-db|deploy-relay|selectedNetwork' app libs scripts .github` → **nothing**. (**Metric corrected during build:** the original grep had no `--glob '!**/*.md'`, so it matched 47 board cards in `libs/aven-board/board/**` — the shipped-work archive, which *must* name what it recorded — plus this card itself and the "Removed by 0121" list in `scripts/README.md`. Documentation of deleted things is not a dead reference; the intent was always live code.)
- [x] Frontend gates green — `bun --cwd app run check` **exit 0** (svelte-check 713 files, 0 errors, down from 2123) and `bun run lint` **exit 0**.
- [x] Rust builds without the deleted crates — `cargo check --manifest-path app/src-tauri/Cargo.toml` **exit 0**.
- [x] The app opens straight into avenCITY — `bun run dev:app:mac` serves on :1420 with document title **avenCITY**, the island renders, and there is no Select Network step; the dev log carries only the game's three.js output. Screenshot in the transcript.
...
- [x] App release CI survives, server CI does not — `rg -c 'release-macos|release-ios|release-linux' .github/workflows/release-next.yml` → **6 matches** (3 job names + 3 `needs:`/comment refs); `rg -l 'deploy-relay|deploy-auth|sprite|flyctl' .github/workflows/` → **nothing**.
- [x] The icon and logo are untouched — `git diff --stat next -- app/src-tauri/icons app/static/aven-logo.svg app/static/app-icon.png app/static/favicon.png` → **empty**. (Baseline is `next`, NOT `main`: `main` predates the brain-icon work, so diffing against it would always show a change.)
...
- `2026-07-31` — **Steps 4–10 done; every criterion checked. Moved build → review.** 259 619 lines deleted across 1 146 files; `libs/` 17 → 2; svelte-check 2123 → 713 files. Four things the spec did not anticipate, all fixed in place rather than worked around: (1) `build.rs` was *entirely* workarounds for deps that no longer exist (ggml archive dedupe, sherpa iOS link directives) and failed to compile once `ar` left build-dependencies; (2) `capabilities/default.json` and `tauri.conf.json` still granted permissions and bundled resources for deleted plugins, which fails at `generate_context!`, not at `cargo check` of the source; (3) `dev-app-desktop.ts` imported the deleted `aven-server.ts` — `dev:app:mac` died instantly, so the last criterion was unreachable until it was rewritten; (4) the three release scripts the card promised to keep all provisioned onnxruntime and passed AI feature flags, so "keep the app CI" meant editing them, not just leaving them alone. Also swept `skills/`, `docs/` and `app/tests/`, which the card never listed. **Residual, deliberately not done:** `scripts/tauri-ios-asc.ts` still carries dead comments and an Accelerate/dylib-stripping path for AI deps that no longer exist — inert (the strip is `force: true`, the framework patch is additive) but worth a follow-on card. The iOS/macOS release scripts are unverified end-to-end here: proving them needs real signing credentials and a TestFlight upload.
