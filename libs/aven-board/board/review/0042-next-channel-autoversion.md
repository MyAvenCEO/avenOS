...
NO upload, NO deploy.** Milestone 2 is a separate card ([[0043-next-channel-release-deploy]]).
...
- `2026-06-14` — **Real CI run (live, on `next`).** Pushed the workflow on a `next` branch and triggered it for real. First run did derive → stamp (19 files) → changelog → commit → push-commit automatically, then failed at `gh release create`: the tag was local-only because `git push --follow-tags` skips **lightweight** tags. Fixed `release-next.ts` to create an **annotated** tag and push the ref + tag separately (commit `c7e447da`). Completed `v26.6.1-next.1` non-destructively, then a second fully-automated run produced **`v26.6.1-next.2`** end-to-end (tag pushed + GitHub prerelease created), with the counter correctly incrementing `next.1 → next.2`. Both releases live. Also added `--dry-run` to `release-next.ts` and a Tauri `getVersion()` app-version line in the settings aside (separate from this card's metric).
...
- `2026-06-14` — Discovery: surveyed versioning (all `0.0.1`, zero tags, no changelog tooling, commits already conventional). Carved Milestone 1 (tags + changelog only) here; Milestone 2 (release + TestFlight + node deploy) → ideate card 0043. Moved ideate → discover.
