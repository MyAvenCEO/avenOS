...
- `2026-08-12` — **Follow-up (Samuel): Vibe-Vokabular getilgt + finale Klassen-Vereinigung.** Es gibt nur noch ACTORS: `VibeActor` in `Actor` verschmolzen (logic im Manifest ⇒ Session + state + generische Adapter, sonst klassisch), Manifest flachgezogen (`logic`/`source`/`view`/`style`/`views` statt vibe/vibes-Bundle), `VibeSession`→`LogicSession`, `VibeEvent`→`ActorEvent`, `vibeState`→`state`, Ordner `vibes/`→`views/`, Test-Datei `sandbox.test.ts`. Interface-Vereinigung im Explorer: Contract + Methods = EINE Interface-Sektion (Dataflow-Zeile immer sichtbar — 'none — reachable by message only' —, Graph immer, Entries darunter). Membrane-Tests laufen jetzt über echte Logic-Actors statt Subklassen. 51 Tests grün, Check 0, live verifiziert.
