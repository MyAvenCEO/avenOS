...
- `2026-08-12` — **BUILD COMPLETE — alle Goal-Kriterien grün** (`tests/instances.test.ts` 7/7, Suite 58/58, Check 0). (1) IDENTITY: `Actor.uuid` + `instanceName`; Bus speichert per UUID, Namen sind ein Index (Template-Name → Default-Instanz). (2) DISCOVERY: registry_list-Zeilen tragen uuid/template/name/default; Adressen kommen aus der Registry. (3) ENGINE-PRIMITIVE: spawn/dispose als Registry-Entries (Events SPAWN/DISPOSE, Caps auf bus.spawn/dispose); Default-Instanz unentsorgbar; divergierende Reduces bewiesen. (4) WINDOWS: `instance-windows.ts` (pure, getestet) + onSpawned/onDisposed-Hooks; erstes Fenster nimmt die Bühne. DAZU (Samuel, mid-build): Adressierung abject-treu als `to` (uuid ODER Name) auf JEDEM Tool-Spec, `to` wird nur als Routing gestrippt, wenn der Adressat die Methode behandelt (sonst Daten — dispose!); das Primitiv selbst als universelles `send(to, method, payload)`-Tool exponiert — benannte Tools sind Schema-Zucker darüber. Explorer: Instances-Lens = Liste mit Spawn/Dispose, Nav uuid-gekeyt mit Instanznamen, reaktives meshActors. Live verifiziert auf 5182: spawn "umzug" per UI → Fenster öffnet, Klick landet in der Instanz. HITL offen: Voice-Spawn in der nativen App.
