...
- **`/idea`** — capture ideas as `board/idea/NNNN-slug.md` from `templates/work-item.md`.
- **`/discovery`** — `git mv` into `board/discovery/`, interview to uncover the real
  goal, make it **measurable**, and fill out `templates/plan.md` (Approach,
  Acceptance criteria, Verification).
- **`/goal`** — execute the smallest change; `git mv` into `board/review/` when built.
...
- **`/ship`** — on human approval, `git mv` into `board/ship/` as the shipped record.
...
**Goal-driven hand-off:** every item past `idea/` has a `goal` — one measurable
...
```
/board-goal <item-ref>     # resolves the item, executes + reviews, moves columns
