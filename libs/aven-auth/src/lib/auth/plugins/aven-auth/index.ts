...
async function getSiteConfig(ctx: { context: { adapter: { findOne: Function } } }): Promise<SiteConfigRow | null> {
...
async function ensureSiteConfigRow(ctx: { context: { adapter: { findOne: Function; create: Function } } }) {
...
async function findInviteByToken(ctx: { context: { adapter: { findOne: Function } } }, token: string): Promise<InviteRow | null> {
	return (await ctx.context.adapter.findOne({
		model: 'selfInvite',
		...
		endpoints: {
			...
			inviteCreate: createAuthEndpoint(
				...
				async (ctx) => {
					...
					if (!session?.user?.id) {
						throw APIError.fromStatus('UNAUTHORIZED', { message: 'Authentication required', status: 401 })
					}
					const site = await getSiteConfig(ctx)
				...
				async (ctx) => {
					...
					if (!existingAccount) {
						...
						if (flow === 'invite') {
							...
							if (!token) {
								throw APIError.fromStatus('BAD_REQUEST', { message: 'inviteToken is required', status: 400 })
							}
							const invite = await findInviteByToken(ctx, token)
							if (!invite || !inviteIsValid(invite)) {
								throw APIError.fromStatus('BAD_REQUEST', { message: 'Invalid or expired invite', status: 400 })
							}
							inviteId = invite.id
						}
					}
			...
			verify: createAuthEndpoint(
				...
				async (ctx) => {
					...
					if (!parsed || parsed.did !== did) {
						throw APIError.fromStatus('UNAUTHORIZED', { message: 'Invalid challenge message', status: 401 })
					}
					if (parsed.domain !== options.domain) {
						throw APIError.fromStatus('UNAUTHORIZED', { message: 'Challenge domain mismatch', status: 401 })
					}
					if (parsed.network !== options.networkSeed) {
						throw APIError.fromStatus('UNAUTHORIZED', { message: 'Challenge network mismatch', status: 401 })
					}
					...
					if (!challenge || challenge.usedAt || challenge.message !== ctx.body.message || challenge.did !== did || challenge.flow !== flow) {
						throw APIError.fromStatus('UNAUTHORIZED', { message: 'Invalid or used challenge', status: 401 })
					}
					...
					const { providerId, accountId } = providerIdForDid(did)
					const existingAccount = await ctx.context.internalAdapter.findAccountByProviderId(accountId, providerId)
					...
					if (!user) {
						if (flow === 'bootstrap') {
							...
							if (challenge.inviteId && challenge.inviteId !== invite.id) {
								throw APIError.fromStatus('BAD_REQUEST', { message: 'Invite mismatch', status: 400 })
							}
							await ctx.context.adapter.update({
								model: 'selfInvite',
						...
						if (!user) {
							throw APIError.fromStatus('INTERNAL_SERVER_ERROR', { message: 'Failed to create user', status: 500 })
						}
					...
					if (!session) {
						throw APIError.fromStatus('INTERNAL_SERVER_ERROR', { message: 'Failed to create session', status: 500 })
					}
