    ...
    pub fn get_or_load(
        ...
        for branch_name in branches {
            ...
            if let Ok(Some(loaded)) = storage.load_branch(id, &bn) {
                ...
                // `LoadedBranch::tails` is the serialized **frontier** read from the
                // `…:tips` storage key (MemoryStorage/SurrealKV both mis-name it `tails`).
                //
                // The previous implementation inserted *every* commit id into `Branch::tips`,
                // while also copying this same frontier set into `Branch::tails`. The latter is
                // *truncation topology* (`add_commit` enforces descendants-of-tails), so benign
                // histories were rejected as `ParentNotFound` after reload (classic symptom:
                // row readable, update/delete fails).
                //
                // Truncate-branch metadata is overloaded onto the same `…:tips` key in the
                // SurrealKV backend; reloading that edge case loses enough information to fully
                // reconstruct both fields. Omitting truncation on load restores normal CRUD until
                // jazz-tools persists those separately (see MemoryStorage truncation tests).
                let mut tips: SmolSet<[CommitId; 2]> = loaded.tails.iter().copied().collect();
                if tips.is_empty() && !commits.is_empty() {
                    // Tips key missing or empty — frontier = commits never referenced as a parent.
                    ...
                    for cid in commits.keys().copied() {
