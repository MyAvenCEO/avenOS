...
///
/// **Naming:** [`LoadedBranch::tails`] is the persisted **tips frontier**
/// deserialized from storage’s `:tips` key (see `branch_tips_key` in backends).
/// It is *not* [`crate::object::Branch::tails`] (truncation topology); that lives
/// only in memory unless/until backends store it under a distinct key.
