...
pub enum QueryError {
    ...
    /// AvenOS-only escape hatch: surface the real `object_manager::Error`
    /// (`BranchNotFound`, `ParentNotFound`, `StorageError`, …) when a write
    /// path would otherwise collapse it into `ObjectNotFound`. Lets callers
    /// distinguish "row missing" from "branch/tip state is inconsistent".
