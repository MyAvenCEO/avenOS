...
import { type AvenDbRow, createIdentity } from '$lib/avendb/api'
			...
			<li>
				...
				>
					<span
						...
						>{t('identities.identityLabel')}</span
					>
					<span class="text-base font-medium tracking-tight group-hover:text-accent-foreground"
						>{row.name || t('common.unnamed')}</span
					>
					<span
						class="text-muted-foreground font-mono text-[11px] group-hover:text-accent-foreground/85"
						>{sparkSubtitle(row)}</span
					>
				</button>
			</li>
		{/each}
				...
				{#if creatingType === type}
					...
					>
						...
						<div class="flex items-center gap-2">
							...
							>
								{creating ? t('identities.creating') : t('common.create')}
							</button>
							<button
								type="button"
...
<div class="flex min-h-0 flex-1 flex-col overflow-y-auto">
	<div class="mx-auto flex w-full max-w-3xl flex-col gap-8 px-4 py-8 sm:px-6">
		...
		{:else if identitiesStore.error}
			<p
				class="text-destructive border-destructive/40 bg-destructive/10 rounded-lg border px-3 py-2 text-sm"
				role="alert"
			>
				{identitiesStore.error}
			</p>
		{:else if loading}
