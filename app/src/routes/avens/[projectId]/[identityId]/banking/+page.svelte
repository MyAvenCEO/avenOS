	...
	<header class="space-y-2">
		<div class="flex flex-wrap items-baseline gap-x-3 gap-y-1">
			...
			<span class="text-muted-foreground text-xs"
				>{t('avens.banking.providerCount', { count: PROVIDERS.length })}</span
			>
		</div>
	...
	<div class="flex min-h-0 flex-1 flex-col gap-4 lg:flex-row">
		...
		<aside class="lg:order-last lg:w-56 lg:shrink-0">
			<div class="border-input overflow-hidden rounded-xl border bg-card/40">
				<p
					class="bg-muted/40 text-muted-foreground border-b border-border/60 px-3 py-2 text-[11px] font-semibold tracking-wide uppercase"
				>
		...
		<!-- Transactions -->
		<div
			class="border-input flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border bg-card/40"
		>
			<div
				class="bg-muted/40 flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1 border-b border-border/60 px-4 py-2.5"
			>
				<span class="text-sm font-semibold">
					...
					<span class="text-muted-foreground text-[11px] font-normal"
						>· {t('avens.banking.txnCount', { count: filtered.length })}</span
					>
				</span>
			...
			<ul class="divide-border/40 min-h-0 flex-1 divide-y overflow-auto">
				{#each filtered as tx (tx.id)}
					...
					<li class="flex items-center gap-3 px-4 py-2.5">
						<span class="text-muted-foreground w-20 shrink-0 text-[11px] tabular-nums"
						...
						<span
							class="shrink-0 text-right text-sm font-semibold tabular-nums {tx.amount >= 0 ? 'text-emerald-600' : 'text-red-600'}"
						>
