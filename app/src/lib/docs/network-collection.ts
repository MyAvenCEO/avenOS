...
const founderModules = import.meta.glob('@avenos/docs/network/founders/*.md', {
	query: '?raw',
	import: 'default',
	eager: true,
}) as Record<string, string>

const developerModules = import.meta.glob('@avenos/docs/network/developers/*.md', {
	query: '?raw',
	import: 'default',
	eager: true,
}) as Record<string, string>
...
function buildMeta(path: string, raw: string, group: DocGroup): NetworkDocMeta {
...
function buildGroup(modules: Record<string, string>, group: DocGroup): NetworkDocMeta[] {
...
export const founderDocs: NetworkDocMeta[] = buildGroup(founderModules, 'founders')
export const developerDocs: NetworkDocMeta[] = buildGroup(developerModules, 'developers')
export const allNetworkDocs: NetworkDocMeta[] = [...founderDocs, ...developerDocs]

export const firstFounderSlug: string = founderDocs[0]?.slug ?? ''
export const firstDeveloperSlug: string = developerDocs[0]?.slug ?? ''

export function getNetworkDoc(group: DocGroup, slug: string): NetworkDocMeta | undefined {
