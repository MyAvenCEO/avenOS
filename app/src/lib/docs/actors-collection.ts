/**
 * Markdown chapters under docs/actors/developers/
 */
export type ActorsDocMeta = {
	slug: string
	order: number
	title: string
	raw: string
...
const developerModules = import.meta.glob('@avenos/docs/actors/developers/*.md', {
	query: '?raw',
	import: 'default',
	eager: true,
...
function buildMeta(path: string, raw: string): ActorsDocMeta {
...
export const actorDocs: ActorsDocMeta[] = Object.entries(developerModules)
	.filter(([path]) => DOC_KEY.test(path.split('/').pop() ?? ''))
	.map(([path, raw]) => buildMeta(path, raw as string))
	.sort((a, b) => a.order - b.order || a.slug.localeCompare(b.slug))

export const firstActorDocSlug: string = actorDocs[0]?.slug ?? ''

export function getActorDoc(slug: string): ActorsDocMeta | undefined {
	return actorDocs.find((d) => d.slug === slug)
}
