...
<div class="flex min-h-0 flex-1 flex-col overflow-y-auto p-4">
	<div class="mx-auto flex w-full max-w-4xl flex-col">
		...
		{#if error}
			<p class="text-destructive mt-3 text-center text-sm">
				...
				>
					...
					{#if act === 'current'}
						<div class="mt-5 flex flex-col gap-2">
							<div
								class="border-border text-muted-foreground rounded-full border px-4 py-2.5 text-center text-sm font-medium"
							>
								{t('mainnet.pricing.currentPlan')}
							</div>
							{#if activeSub?.cancelAtPeriodEnd}
								<p class="text-muted-foreground text-center text-[11px] leading-relaxed">
									{t('mainnet.pricing.cancelsOn', {
										date: fmtDate(activeSub.endsAt ?? activeSub.currentPeriodEnd)
									})}
								</p>
								<button
									type="button"
									class="text-primary text-center text-[11px] font-semibold underline-offset-2 hover:underline disabled:opacity-50"
									onclick={() => void doResume()}
									disabled={manageBusy}
								>
									{manageBusy ? t('mainnet.pricing.working') : t('mainnet.pricing.resume')}
								</button>
							{:else if activeSub && confirmCancel}
								<p class="text-muted-foreground text-center text-[11px] leading-relaxed">
									{t('mainnet.pricing.cancelConfirm', { date: fmtDate(activeSub.currentPeriodEnd) })}
								</p>
								<div class="flex gap-2">
									<button
										type="button"
										class="border-destructive/40 text-destructive hover:bg-destructive/10 flex-1 rounded-full border px-3 py-1.5 text-xs font-semibold transition-colors disabled:opacity-50"
										onclick={() => void doCancel()}
										disabled={manageBusy}
									>
										{manageBusy ? t('mainnet.pricing.working') : t('mainnet.pricing.cancelYes')}
									</button>
									<button
										type="button"
										class="border-border hover:bg-card flex-1 rounded-full border px-3 py-1.5 text-xs font-semibold transition-colors disabled:opacity-50"
							...
							{:else if activeSub}
								<button
									type="button"
									class="text-muted-foreground hover:text-foreground text-center text-[11px] font-semibold underline-offset-2 hover:underline"
		...
		<p class="text-muted-foreground mx-auto mt-5 max-w-lg text-center text-[11px] leading-relaxed">
			{t('mainnet.pricing.footnote')}
		</p>

		<!-- My subscriptions & orders — entirely in our own UI -->
		{#if billing && (activeSubs.length > 0 || billing.orders.length > 0)}
			<div class="mt-8">
				<h3 class="font-display text-foreground text-base font-medium tracking-tight">
