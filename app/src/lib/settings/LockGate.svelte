...
const gated = $derived(browser && isTauriRuntime() && $deviceSession.kind === 'locked')
...
function applyVaultList(list: VaultListEntry[]): void {
	...
	if (list.length === 0) {
		...
		mode = 'pick'
		selectedSlug = list.find((v) => v.hasIdentityBlob)?.usernameSlug ?? list[0]?.usernameSlug
	...
	>
		...
		>
			<div class="max-w-md space-y-2 text-center">
				...
				<h1 id="lock-title" class="avenos-wordmark">
			...
			{#if err}
				<p class="text-destructive max-w-lg text-center text-xs leading-relaxed" role="alert">
					{err}
				</p>
			{/if}
			...
			{:else if mode === 'pick'}
				<div class="flex w-full max-w-md flex-col gap-3">
					<ul
						class="max-h-[40vh] space-y-2 overflow-y-auto"
						role="list"
						aria-label={t('lockGate.chooseYourselfList')}
					>
					...
					<button type="button" class={outlineBtnClass} disabled={loading} onclick={startCreate}>
				...
				<div class="flex w-full max-w-md flex-col gap-5">
					<div>
						...
						{#if resolvingDeviceLabel}
							<p class="text-muted-foreground mb-2 text-center text-xs" aria-live="polite">
					...
					<button type="button" class={outlineBtnClass} disabled={loading} onclick={backCreateStep}>
						{t('common.back')}
					</button>
				</div>
			{/if}
