...
import { toStorageId, type VaultSecretKind, vaultSecretTitleKey } from '$lib/vault/secret-kind'
import {
...
<section class="flex h-full min-h-0 flex-1 flex-col px-4 py-4 sm:px-6 sm:py-6">
	...
	{:else}
		{#key kind}
			<AvenUiView {shell} {source} onEvent={handleEvent} containerName="aven-ui-vault-secrets" />
		{/key}
	{/if}
</section>
