import { MessageBus } from '$lib/actors/bus'
import type {
	ClientArtifactDraft,
	ClientEvidence,
	DocumentActorResult,
...
import { MAX_MODEL_PAGES } from '$lib/actors/document-model'
import type {
...
const roleFor = (artifact: MaterializedArtifact): string => {
	if (artifact.typeKey === 'core.file-inspection') return 'inspection'
	if (artifact.typeKey === 'docs.page') return 'page'
	if (artifact.typeKey === 'docs.extracted-text') return 'text'
	if (artifact.typeKey === 'docs.text-layout') return 'layout'
	if (artifact.typeKey === 'core.content-description') return 'description'
	if (artifact.typeKey === 'core.document-classification') return 'document-classification'
	if (artifact.typeKey === 'bookkeeping.invoice-candidate') return 'candidate'
	if (artifact.typeKey === 'bookkeeping.invoice-details') return 'details'
	if (artifact.typeKey === 'banking.account-statement-candidate') return 'candidate'
	if (artifact.typeKey === 'core.content-classification') {
		return artifact.payload.subjectLevel === 'page'
			? 'page-classification'
...
export class DocumentProcessingRuntime {
	...
	readonly #modelStatus?: () => Promise<{ available: boolean; maxPages: number }>
	...
	constructor(
		...
		modelStatus?: () => Promise<{ available: boolean; maxPages: number }>
	) {
	...
	async #run(source: DocumentSource): Promise<ArtifactProcessingPresentation> {
		const presentation: ArtifactProcessingPresentation = {
			...
			summary: null,
			metadata: { execution: 'client-actors' },
			warnings: [],
			stages: [],
			derivedArtifacts: []
		}
