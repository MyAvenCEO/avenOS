...
interface ArtifactContent { mediaType: string; base64: string }

let { row, initialArtifactId, onclose }: {
	row: LibraryRow
...
onMount(() => {
	...
	return () => { alive = false; selectionRevision++ }
})
</script>

<div class="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-2xl border border-border bg-surface-raised">
	<header class="flex shrink-0 items-start justify-between gap-3 border-border border-b p-3">
		<div class="min-w-0">
			...
			<p class="mt-1 font-mono text-[length:var(--fs-micro)] text-foreground/45">{sourceId.slice(0, 8)} · {row.mediaType}</p>
		</div>
		<button type="button" class="shrink-0 rounded border border-border px-2 py-1 text-xs" onclick={onclose} aria-label="Vorschau schließen">Schließen</button>
	</header>
	<div class="flex shrink-0 flex-wrap items-center gap-2 border-border border-b p-3">
		<button type="button" aria-pressed={selectedId === sourceId}
			class="rounded-full border border-border px-2.5 py-1 text-xs {selectedId === sourceId ? 'bg-primary text-primary-foreground' : ''}"
			onclick={() => void selectArtifact(sourceId)}>Original</button>
		{#each row.representations.filter((artifact) => ['bookkeeping.invoice-details', 'bookkeeping.invoice-candidate', 'banking.account-statement-candidate', 'bookkeeping.invoice-validation', 'banking.statement-validation'].includes(artifact.typeKey)) as artifact (artifact.artifactId)}
			<button type="button" aria-pressed={selectedId === artifact.artifactId}
				...
				onclick={() => void selectArtifact(artifact.artifactId)}>{representationLabels[artifact.typeKey]}</button>
		{/each}
		<button type="button" class="ml-auto rounded-full border border-border px-2.5 py-1 text-xs"
	...
	<div class="flex min-h-0 flex-1 flex-col lg:flex-row">
		{#if selectedId !== sourceId}
			<section class="flex min-h-[12rem] min-w-0 flex-1 flex-col overflow-auto border-border border-b lg:border-r lg:border-b-0" aria-label="Extrahierte Angaben">
				{#if envelopeLoading}<p class="p-4 text-sm" role="status">Angaben werden geladen …</p>{/if}
				{#if envelopeFailure}<p class="p-4 text-error-ink text-sm" role="alert">{envelopeFailure}</p>{/if}
				{#if envelope}
					<ArtifactSemanticViewer typeKey={String(envelope.typeKey ?? row.artifactType)} payload={envelope.payload}
						{evidence} {activeEvidence} onEvidence={(edge) => activeEvidence = edge} />
				{/if}
			</section>
		{/if}
		<section class="flex min-h-[12rem] min-w-0 flex-1 flex-col overflow-hidden bg-surface-sunken/25" aria-label="Quelldokument">
			{#if sourceLoading}<p class="p-4 text-sm" role="status">Dokument wird geladen …</p>{/if}
			{#if sourceFailure}<p class="p-4 text-error-ink text-sm" role="alert">{sourceFailure}</p>{/if}
			{#if sourceContent}
