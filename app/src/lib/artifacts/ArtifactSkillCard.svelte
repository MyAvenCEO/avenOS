...
>
	...
	{/if}
	{#if presentation.structure.branches || presentation.structure.reviews || presentation.structure.openGoals || presentation.structure.collections}
		<div class="mt-2 flex flex-wrap gap-1 text-[length:var(--fs-micro)] text-foreground/45">
			{#if presentation.structure.branches}<span>⑂ {presentation.structure.branches}</span>{/if}
			{#if presentation.structure.reviews}<span>◐ {presentation.structure.reviews}</span>{/if}
			{#if presentation.structure.openGoals}<span>✳ {presentation.structure.openGoals}</span>{/if}
			{#if presentation.structure.collections}<span>≋ {presentation.structure.collections}</span>{/if}
		</div>
	{/if}
	{#if committedAt}
