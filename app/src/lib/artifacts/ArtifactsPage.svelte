			...
			>
				...
				{:else}
					<div class="min-h-0 flex-1 overflow-auto">
						<table aria-label="Artifact lineage" class="w-full min-w-[61rem] table-fixed text-left text-xs">
							<colgroup>
								<col class="w-[17rem]">
								<col class="w-[13rem]">
								<col class="w-[8rem]">
							...
							<thead class="sticky top-0 z-10 bg-surface-raised text-[0.5625rem] text-foreground/40 uppercase tracking-wide">
							...
							<tbody>
								{#each treeRows as row (row.artifact.artifactId)}
									...
									>
										<td class="p-0">
											<div class="flex min-w-0 items-center" style:padding-left={`${row.depth * 16 + 6}px`}>
												{#if row.hasChildren}
													<button
														type="button"
													...
													>
														<span class="transition-transform {collapsedIds.has(artifact.artifactId) ? '' : 'rotate-90'}">›</span>
													</button>
												{:else}
													<span class="grid size-6 shrink-0 place-items-center text-foreground/20">·</span>
												{/if}
												<button
													type="button"
												...
												>
													<span class="block truncate font-medium">{artifactTypeLabel(artifact.typeKey)}</span>
													<span class="block truncate font-mono text-[0.5625rem] text-foreground/35">{artifact.artifactId}</span>
												</button>
											</div>
										</td>
										<td class="truncate px-2 py-2 font-mono text-[0.625rem]" title={artifact.typeKey}>
											{artifact.typeKey}<span class="text-foreground/35">@{artifact.typeVersion}</span>
										</td>
										<td class="truncate px-2 py-2 font-mono text-[0.625rem] text-foreground/55" title={artifact.localKey}>{artifact.localKey}</td>
										<td class="truncate px-2 py-2 font-mono text-[0.625rem] text-foreground/55">{outputLabel(artifact.output)}</td>
										<td class="px-2 py-2 text-right font-mono text-[0.625rem]" title={artifact.inputs.map((input) => `${input.role}:${input.ordinal} → ${input.artifactId}`).join('\n')}>
											<span class={row.missingParentCount ? 'text-warning-ink' : 'text-foreground/55'}>
												{row.parentCount || '—'}{row.missingParentCount ? ` · ${row.missingParentCount}?` : ''}
											</span>
										</td>
										<td class="px-2 py-2 text-right font-mono text-[0.625rem] text-foreground/55">#{artifact.scopeSequence}</td>
										<td class="truncate px-2 py-2 text-[0.625rem] text-foreground/55" title={new Date(artifact.committedAt).toISOString()}>{new Date(artifact.committedAt).toLocaleString('de-DE')}</td>
										<td class="truncate px-2 py-2 font-mono text-[0.625rem] text-foreground/55" title={artifact.runId ?? 'Root publication'}>{shortId(artifact.runId)}</td>
									</tr>
								{/each}
							</tbody>
						</table>
				...
				{#if selected}
					<header class="border-border border-b px-4 py-3">
						...
						<div class="mt-1 flex items-center gap-2">
							...
							{/if}
							<span class={selected.inputs.length ? 'text-foreground/40 text-xs' : 'ml-auto text-foreground/40 text-xs'}>#{selected.scopeSequence}</span>
						</div>
					</header>
					{#if envelopeLoading}
