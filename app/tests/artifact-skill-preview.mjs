...
const sourcePort = {
	schema: 'ceo.aven:schema:docs:file@1', type: { key: 'core.file', version: 1 },
	predicate: 'ceo.aven.docs.file(F)', role: 'source', cardinality: 'one'
}
const resultPort = {
	schema: 'ceo.aven:schema:docs:inspection@1',
	type: { key: 'core.file-inspection', version: 3 },
	predicate: 'ceo.aven.docs.file_inspection(F, I)', role: 'inspection', cardinality: 'one'
}
const definition = {
	version: 2, name: 'Inspect project updates', inputs: { source: sourcePort },
	parametersSchema: {
		type: 'object', properties: { detail: { type: 'boolean' } }, required: [],
		additionalProperties: false, maxProperties: 1
	},
	steps: [{
		id: 'inspect', kind: 'invoke', label: 'Inspect project updates',
		capabilityId: 'ceo.aven:capability:docs.ingest:document_inspect@1',
		inputs: { source: { kind: 'input', port: 'source' } },
		parameters: { detail: { kind: 'parameter', name: 'detail' } }, outputs: { inspection: resultPort }
	}],
	outputs: {
		inspection: { ...resultPort, from: { kind: 'step', stepId: 'inspect', port: 'inspection' } }
	},
	policy: {
		maxInvocations: 8, maxDepth: 4, maxMembers: 32, maxConcurrentChildren: 2,
		allowModel: false
	}
}
const file = {
...
const skill = {
	...
	typeKey: 'studio.skill', typeVersion: 2, artifactSha256: 'skill-digest',
...
try {
	...
	await page.getByRole('region', { name: 'Skill settings' }).getByText(/detail/).waitFor()
