...
//! against upstream when Supertonic ships a new model revision. The local
//! changes are bounded/non-spinning ORT sessions and cancellable RunOptions;
//! integration code remains in `mod.rs`.
...
/// ahead of playback without allowing each model stage to claim every core.
/// The configured pools sleep between runs.
...
impl TextToSpeech {
    ...
    pub fn batch(
		...
		let run_options = RunOptions::new()?;
		self._infer(text_list, lang_list, style, total_step, speed, &run_options)
    }
}
