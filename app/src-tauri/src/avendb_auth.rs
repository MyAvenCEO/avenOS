//! Peer identity for AvenOS ACC + AvenDb session shim.
...
/// `aven_db::did_key` (so the app, the device engine, and the `aven-p2p` transport
/// handshake all decode the same bytes — see board `0004-aven-node-mini`).
/// Re-exported here so existing `crate::avendb_auth::…` call sites are unchanged.
pub use aven_db::did_key::{ed25519_public_from_signer_did, signer_did_from_ed25519};

/// Deterministic avendb `PeerId`-style fingerprint (not a JWT claim for ACC).
#[must_use]
pub fn client_uuid_from_ed_pubkey(pubkey: &[u8; 32]) -> uuid::Uuid {
	use sha2::{Digest, Sha256};
	let mut digest = Sha256::new();
	digest.update(b"ceo.aven.os/avendb/client-id-v1");
