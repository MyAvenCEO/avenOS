...
import { installStudioRuntimeActors, STUDIO_EMAIL_BRIEF_CAPABILITY } from '../src/studio-runtime.js'
...
const security: PlanRunSecurityContext = {
	...
	establishedBy: 'test-boundary',
	authorizedAt: '2026-09-16T12:00:00Z'
}

describe('production document Studio inventory policy', () => {
	test('advertises the actually installed email-brief factory with scoped planning authority', async () => {
		const installations = new TrustedActorInstallations()
		await installStudioRuntimeActors(installations)
		const source = createDocumentCatalogSource(scope, installations)
		...
		const brief = page.entries.find((entry) => entry.capabilityId === STUDIO_EMAIL_BRIEF_CAPABILITY)
		expect(registry.offers).toHaveLength(1)
		expect(brief).toMatchObject({ readiness: 'needs-input', canAuthor: true, canPlan: true })
		expect(
			page.entries
				.filter((entry) => entry.capabilityId !== STUDIO_EMAIL_BRIEF_CAPABILITY)
				.every((entry) => entry.readiness === 'contract-incomplete' && !entry.canPlan)
		).toBe(true)
	})
	test('discovers release-owned methods but never advertises generic execution', async () => {
		...
		expect(
			page.actors.every((actor) => actor.kind === 'operations' && actor.visibleOperationCount === 1)
		).toBe(true)
		expect(page.readyVisibleCount).toBe(0)
		expect(page.entries.every((entry) => entry.readiness === 'contract-incomplete')).toBe(true)
		expect(
			page.entries.every((entry) => entry.reasonCodes.includes('INSTALLATION_UNAVAILABLE'))
		).toBe(true)
		expect(
			page.entries.every((entry) => !entry.canAuthor && !entry.canPlan && !entry.canInvokeNow)
		).toBe(true)
		const capability = registry.definitions[0]!.capabilities[0]!
		const plan = await authorizer.decide({
			action: 'plan',
			principal: security.principal,
			access: security.access,
			definitionRef: registry.definitions[0]!.ref,
	...
	test('a different customer or unauthenticated principal cannot enumerate the package', async () => {
		...
		const other = await new StudioCatalog().page({
			registry,
			authorizer,
			principal: security.principal,
			access: { tenantId: 'customer-b' },
			installationFor: source.installationFor
		})
