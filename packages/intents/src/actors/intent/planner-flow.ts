...
export function buildPlannerPrompt(state: IntentActorState, helpers: Pick<IntentPlannerFlowHelpers, "sanitizeJson" | "bounded" | "listIntentToolCatalog">): string {
  ...
  const prompt = [
    ...
    "1a. If the request depends on the current local machine, operating system, shell environment, current time/date/timezone, current user identity, filesystem contents, installed tools, network access from this machine, or any other runtime-local fact, do not answer from general knowledge. Use an appropriate tool first, preferably shell.execute for command-line verifications such as date, whoami, pwd, ls, env, or similar local inspection commands.",
    ...
    "For local factual questions like 'what time is it', 'who am I', 'what directory is this', 'what files are here', or similar machine-state questions, prefer shell.execute with a minimal read-only command rather than answering directly.",
