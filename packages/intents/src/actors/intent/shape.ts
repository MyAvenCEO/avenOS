...
import type { LlmCapabilityRequirements, LlmResult } from "llm-contracts";
...
import type { IntentActorState, IntentToolRunState, IntentsRouterState } from "./types.ts";
...
export const intentsRouterShape = defineActorShape({
  ...
  messages: {
    ...
    routeClarificationAnswered: msg({
      communicationId: field.string(),
      routerClarificationId: field.string(),
      answer: field.json(),
    }),
  ...
  operations: {
    ...
    routeHumanMessage: op({
      title: "Route human message",
      description: "Route a human message to a waiting intent when possible, otherwise create a new intent.",
      mutates: true,
      input: {
        message: field.string({ default: "Hello" }),
        attachments: field.json({ optional: true }),
        plannerRequirements: field.ref<LlmCapabilityRequirements>({ optional: true }),
        plannerModelActorPathOverride: field.string({ optional: true }),
        toolDefaults: field.schema(toolDefaultsSchema, { optional: true }),
...
export const intentActorShape = defineActorShape({
  ...
  state: {
    ...
    goal: field.string(),
    input: field.json({ optional: true }),
    requiresHumanVisibleResult: field.boolean({ default: false }),
    durable: field.boolean({ default: false }),
    externalInputRequest: field.ref<IntentActorState["externalInputRequest"]>({ optional: true }),
    queuedExternalInputs: field.ref<readonly JsonValue[]>({ default: [] as readonly JsonValue[] }),
    ...
    shellContext: field.ref<IntentActorState["shellContext"]>(),
    selectedModels: field.ref<IntentSelectedModels>(),
    plannerSettings: field.ref<IntentActorState["plannerSettings"]>(),
    toolSettings: field.ref<IntentActorState["toolSettings"]>(),
    currentStep: field.integer({ default: 0 }),
    cycleStep: field.integer({ default: 0 }),
    toolRuns: field.integer({ default: 0 }),
    cycleToolRuns: field.integer({ default: 0 }),
    activeToolRunId: field.string({ optional: true }),
    activePlannerRequestId: field.string({ optional: true }),
    missingHumanResultRetries: field.integer({ optional: true }),
    toolValidationFailureFingerprints: field.ref<IntentActorState["toolValidationFailureFingerprints"]>({ optional: true }),
  },
  messages: {
    startIntent: msg({}),
    getIntent: msg({}),
...
export const intentToolRunShape = defineActorShape({
  ...
  messages: {
    ...
    metadataQueryCompleted: msg({
      requestId: field.string(),
      result: field.json(),
    }),
    structuredExtractionCompleted: msg({
      requestId: field.string(),
      result: field.json(),
    }),
    artifactGetDescriptorCompleted: msg({
      requestId: field.string(),
      ref: field.json(),
      descriptor: field.json({ optional: true }),
    }),
    ...
    schemaVersionCompleted: msg({
      requestId: field.string(),
      result: field.json(),
    }),
    shellExecuteCompleted: msg({
      requestId: field.string(),
      result: field.json(),
    }),
  },
