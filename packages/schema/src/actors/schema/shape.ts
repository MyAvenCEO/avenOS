import { buildActorRuntime, collection, defineActorShape, field, msg, op, type DerivedActorRuntime } from "typed-actors";
import type { RegisteredSchemaVersion } from "../../domain.ts";
import type { SchemaActorState, SchemaRegistryState } from "../registry/types.ts";
...
export const schemaRegistryShape = defineActorShape({
  ...
  tree: {
    describeSelf({ state, operations }) {
      return {
        hasChildren: true,
        childCount: state.schemaIds.length,
        operations,
        summary: {
          schemaCount: state.schemaIds.length,
          schemaIds: state.schemaIds,
        },
      };
    },
  },
...
export const schemaActorShape = defineActorShape({
  ...
  tree: {
    children: [
      collection("versions", {
        title: "versions",
        hasChildren: ({ state }) => Object.keys(state.versions).length > 0,
        childCount: ({ state }) => Object.keys(state.versions).length,
      }),
    ],
  },
  present(state) {
