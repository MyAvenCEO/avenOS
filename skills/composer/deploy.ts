 ...
 * `storage.forks.delete(name)` to discard or promote. See skills/composer/README.md.
 *
 * Run:
 *   AWS_ACCESS_KEY_ID=… AWS_SECRET_ACCESS_KEY=… bun skills/composer/deploy.ts
 *   FORK=preview-42 AWS_ACCESS_KEY_ID=… AWS_SECRET_ACCESS_KEY=… bun skills/composer/deploy.ts
 */

import { type AdapterName, buildAdapter } from '@storagesdk/adapters'
...
const T: Record<Loc, Record<string, string>> = {
	en: {
		...
		intro:
			"Static site on a Tigris bucket, deployed via the universal storagesdk.dev API. <code>aven.ceo</code> is the Fly 301 door; <code>www.next.aven.ceo</code> serves every byte direct from Tigris's edge.",
		aboutb:
			'A sample page under a short stable id <code>/en/abc-xyz/</code>, stored as the slash-key <code>en/abc-xyz/</code> and served directly by Tigris.',
		p1: 'Hello, Tigris',
		p1d: 'object storage as a website',
		p1b: "Served straight from Tigris's global edge with free egress."
	},
	de: {
		home: 'Start',
		about: 'Über',
		...
		intro:
			'Statische Seite auf einem Tigris-Bucket, deployed über die universelle storagesdk.dev-API. <code>aven.ceo</code> ist die Fly-301-Tür; <code>www.next.aven.ceo</code> liefert jedes Byte direkt von Tigris.',
		aboutb:
			'Beispielseite unter kurzer stabiler ID <code>/de/abc-xyz/</code>, gespeichert als Slash-Key <code>de/abc-xyz/</code>.',
		p1: 'Hallo, Tigris',
		p1d: 'Object Storage als Website',
		p1b: "Direkt von Tigris' globalem Edge mit kostenlosem Egress."
	}
}
...
const head = (loc: Loc, rel: string, title: string) => {
	const alt =
		LOCALES.map((l) => `<link rel="alternate" hreflang="${l}" href="${HOST}/${l}/${rel}">`).join(
			''
		) + `<link rel="alternate" hreflang="x-default" href="${HOST}/en/${rel}">`
	return (
		`<!DOCTYPE html><html lang="${loc}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">` +
...
const nav = (loc: Loc, active: string) => {
	const t = T[loc],
		B = `/${loc}`,
		other = loc === 'en' ? 'de' : 'en'
	const L = (h: string, lbl: string, k: string) =>
		`<a class="${k === active ? 'on' : ''}" href="${h}">${lbl}</a>`
...
const foot = (loc: Loc) =>
	`<div class="loc">locale <b>${loc}</b> · path <b id="p"></b></div><script>document.getElementById("p").textContent=location.pathname</script></body></html>`
...
function pages(loc: Loc): Record<string, string> {
	...
	return {
		'':
			head(loc, '', t.home) +
			nav(loc, 'home') +
			`<main><span class="tag">next channel · tigris-direct</span><h1>aven.ceo</h1><p class="lede">${t.lede}</p><p>${t.intro}</p><a class="card" href="${B}/abc-xyz/"><div class="t">→ ${t.about}</div><div class="d">/${loc}/abc-xyz/ — short stable id</div></a><a class="card" href="${B}/blog/"><div class="t">→ ${t.blog}</div><div class="d">/${loc}/blog/p1/ — nested</div></a></main>` +
			foot(loc),
		'abc-xyz/':
			head(loc, 'abc-xyz/', t.about) +
			nav(loc, 'about') +
			`<main><a class="back" href="${B}/">←</a><h1>${t.about}</h1><p>${t.aboutb}</p></main>` +
			foot(loc),
		'blog/':
			head(loc, 'blog/', t.blog) +
			nav(loc, 'blog') +
			`<main><a class="back" href="${B}/">←</a><h1>${t.blog}</h1><a class="card" href="${B}/blog/p1/"><div class="t">${t.p1}</div><div class="d">${t.p1d}</div></a></main>` +
			foot(loc),
		'blog/p1/':
			head(loc, 'blog/p1/', t.p1) +
			nav(loc, 'blog') +
			`<main><a class="back" href="${B}/blog/">←</a><h1>${t.p1}</h1><p>${t.p1b}</p></main>` +
			foot(loc)
	}
}
...
for (const loc of LOCALES) {
	for (const [rel, html] of Object.entries(pages(loc))) {
		...
		}) // slash-key (Tigris-direct)
		objs.set(`${loc}/${rel}index.html`, {
			body: html,
			contentType: HTML,
			cacheControl: 'public, max-age=300'
		}) // index.html (fallback)
		objs.set(`${loc}/${rel}`.replace(/\/$/, ''), {
			body: stub(loc, rel),
			contentType: HTML,
			cacheControl: 'public, max-age=300'
		}) // no-slash stub
...
objs.set('404.html', {
	body: `<!DOCTYPE html><meta charset="utf-8"><title>404</title><body style="font-family:system-ui;background:#060f1f;color:#F4EFE6;display:grid;place-items:center;height:100vh;text-align:center"><div><h1 style="font-size:4rem">404</h1><p><a style="color:#7aa2ff" href="/en/">→ home</a></p></div>`,
	contentType: HTML,
	cacheControl: 'public, max-age=300'
})
objs.set('robots.txt', {
...
objs.set('sitemap.xml', {
	body: `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${sitemap.map((u) => `<url><loc>${u}</loc></url>`).join('')}</urlset>`,
...
console.log(
	`deploying ${objs.size} objects → ${FORK ? `fork:${FORK} of ` : ''}bucket:${BUCKET} (${HOST})`
)
let n = 0
...
console.log(
	`done. ${FORK ? `Fork "${FORK}" ready — inspect, then live.forks.delete("${FORK}") to discard.` : 'Live.'}`
)
