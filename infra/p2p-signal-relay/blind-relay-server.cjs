...
/**
 * HyperDHT + blind-relay server for local AvenOS pairing (Node stack, wire-compatible with Hyperswarm).
 *
 * Env:
 *   AVENOS_P2P_SIGNAL_BOOTSTRAP — comma-separated HyperDHT bootstrap entries (Rust stdout `.bootstrap`).
 *   AVENOS_P2P_SIGNAL_KEYS_DIR — directory for persisted 32-byte `relay-hyperdht.seed`.
...
async function main() {
	...
	process.stdout.write(
		JSON.stringify({
			ready: true,
			publicKey: pkHex,
			host: addr.host === '0.0.0.0' ? '127.0.0.1' : addr.host,
			port: addr.port
		}) + '\n'
	)
