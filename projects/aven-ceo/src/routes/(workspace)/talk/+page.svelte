...
<div class={`${workspaceContentClass} flex min-h-0 flex-1 flex-col overflow-hidden`}>
	...
	>
		<div class="flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-hidden">
			<div class="mb-2 flex min-h-6 shrink-0 flex-col gap-0.5">
				...
				<p class="text-[11px] leading-snug opacity-45">
					Everything below is what the server composes for the model on each roundtrip (system
					bundle + tool schemas + transcript).
				</p>
			</div>
			<div
				...
				>
					...
					>
						<span class="flex items-start gap-2">
							<span
								class="talk-ctx-chevron mt-0.5 shrink-0 text-[10px] opacity-45"
								aria-hidden="true"
								>▸</span
							>
								...
								<li class="rounded-xl border border-border/35 bg-white/4 px-3 py-2">
									...
									{#if maiaAgent.sources.systemPrompt.seedPath}
										<span class="mt-1 block text-[9px] opacity-40"
											>seed ·
											<span class="font-mono">{maiaAgent.sources.systemPrompt.seedPath}</span></span
										>
									{/if}
								</li>
								<li class="rounded-xl border border-border/35 bg-white/4 px-3 py-2">
									<span class="tech-label block text-[9px] opacity-45"
										>tools.openAiFunctionSchemas</span
									>
									<a
						...
						<div class="mt-3 grid gap-1 text-[10px] leading-snug opacity-60 sm:grid-cols-2">
							...
							<p class="m-0 sm:col-span-2">
								...
								<span class="ml-1 font-mono"
									>{maiaAgent.systemBundle.snapshotHeadingMarkdownTemplate}</span
								>
							</p>
						</div>
		...
		>
			<div
				class="mb-2 shrink-0 lg:sticky lg:top-0 lg:z-10 lg:bg-background/85 lg:pb-2 lg:backdrop-blur-sm"
			>
				...
				<ul class="m-0 mb-2 list-none space-y-1 border-b border-border/30 pb-2 p-0">
					<li>
						...
						>
							...
							<span class="break-all font-mono text-[9px] tabular-nums opacity-45"
								>{maiaAgent.id}</span
							>
							{#if contextPreview}
							...
							<li>
								...
								>
									...
									<span class="font-mono text-[9px] tabular-nums opacity-45"
										>~{block.estimatedTokens.toLocaleString()}
										tok</span
									>
