...
>
	<div class="flex items-start justify-between gap-3">
		...
		<span class="inline-flex items-center rounded-full border border-tuscan-sun/40 bg-tuscan-sun/20 px-2 py-0.5 font-mono text-[9px] font-bold uppercase tracking-[0.16em] text-tuscan-sun">
			Skill
		</span>
	</div>
	...
	<p class="mt-3 font-serif text-[13px] italic leading-snug text-foreground/58 sm:text-[14px]">
		"{skill.founderScenario.timestamp} — {skill.founderScenario.story.slice(0, 100)}&hellip;"
	</p>

	<div class="mt-4 flex flex-wrap gap-1.5">
		{#each skill.playsWith as { slug } (slug)}
			<span class="inline-flex items-center rounded-full border border-border/40 bg-background/70 px-2 py-0.5 font-mono text-[9px] font-semibold text-foreground/50">
	...
	<div class="mt-5 flex items-center justify-between border-t border-border/30 pt-4">
		<div>
			<p class="font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-tuscan-sun">
				{skill.hero.promiseHoursPerWeek} gespart
			</p>
		</div>
		<span class="font-mono text-[12px] font-semibold text-foreground/55 transition-colors group-hover:text-foreground/80">
			Skill ansehen →
		</span>
	</div>
</a>
