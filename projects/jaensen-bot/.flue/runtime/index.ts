...
import type { IntentDecision, JaensenInput, RunResult, RuntimeDependencies, SkillAction, SkillResult } from './types.js'

function normalizeSkillAction(action: unknown): SkillAction | null {
	if (!action || typeof action !== 'object') return null
	const record = action as Record<string, unknown>
	const skill = record.skill
	const operation = record.operation
	const input = record.input
	if (skill !== 'memory' && skill !== 'ingest' && skill !== 'extract') return null
	if (!isValidOperation(skill, operation)) return null
	return {
		skill,
		operation,
...
function isValidOperation(skill: SkillAction['skill'], operation: unknown): operation is SkillAction['operation'] {
	if (typeof operation !== 'string') return false
	if (skill === 'memory') return operation === 'remember' || operation === 'recall' || operation === 'search'
	if (skill === 'ingest') return operation === 'archive-url' || operation === 'archive-attachment'
	if (skill === 'extract') return operation === 'extract-text' || operation === 'extract-entities'
	return false
}
...
export async function runJaensenTurn(input: JaensenInput, deps: RuntimeDependencies): Promise<RunResult> {
	...
	const routing = await routeInputToIntents(input, activeIntents, deps.generate)
	...
	const intentDecision = await decideIntent(primaryIntent, input, deps)
	...
	const response = await synthesizeFinalResponse(primaryIntent, input, intentDecision, skillResults, deps.generate)
...
export async function loadSkillDocs(baseDir: string): Promise<Record<'memory' | 'ingest' | 'extract', string>> {
	console.log('[jaensen] skills:load-docs', { baseDir })
	return {
		memory: await readFile(join(baseDir, '.flue/skills/memory/SKILL.md'), 'utf-8'),
		ingest: await readFile(join(baseDir, '.flue/skills/ingest/SKILL.md'), 'utf-8'),
		extract: await readFile(join(baseDir, '.flue/skills/extract/SKILL.md'), 'utf-8')
	}
}
...
async function decideIntent(intent: IntentRecord, input: JaensenInput, deps: RuntimeDependencies): Promise<IntentDecision> {
	...
	if (parsed) {
		return enrichIntentDecision(input, intent, {
			summary: parsed.summary || intent.summary,
			status: parsed.status,
			contextUpdates: parsed.contextUpdates ?? {},
			actions: Array.isArray(parsed.actions) ? parsed.actions.map(normalizeSkillAction).filter((action): action is SkillAction => action !== null) : [],
			humanLoop: parsed.humanLoop,
			replyDraft: parsed.replyDraft || 'Received.'
		})
	}
	return enrichIntentDecision(input, intent, heuristicIntentDecision(intent, input))
}
...
async function executeActions(intent: IntentRecord, input: JaensenInput, decision: IntentDecision, deps: RuntimeDependencies): Promise<SkillResult[]> {
	...
	for (const action of decision.actions ?? []) {
		...
		if (action.skill === 'memory') results.push(await runMemorySkill(intent, action, deps))
		if (action.skill === 'ingest') results.push(await runIngestSkill(input, intent, action, deps))
		if (action.skill === 'extract') results.push(await runExtractSkill(intent, action, deps))
	}
	return results
}
