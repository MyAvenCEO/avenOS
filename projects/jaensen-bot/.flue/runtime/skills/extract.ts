...
export async function runExtractSkill(intent: IntentRecord, action: SkillAction, deps: RuntimeDependencies): Promise<SkillResult> {
	const skillDoc = deps.skillRegistry.extract?.doc
	if (!skillDoc) return { skill: 'extract', ok: false, summary: 'Extract skill is not registered' }
...
function buildExtractCommand(sandboxPath: string, contentType?: string): string {
...
limit_output() {
	head -c ${MAX_EXTRACTED_TEXT_CHARS}
}
...
function buildSandboxInputPath(key: string, contentType?: string): string {
	const extension = extname(key).toLowerCase() || extensionFromType(contentType)
	return `input/source${extension === '.bin' ? '' : extension}`
}

function selectExtractedText(workerStdout: string, fallback: string): string {
	const trimmed = workerStdout.trim()
	if (!trimmed) return fallback
	if (trimmed === 'ok' || trimmed.startsWith('worker:')) return fallback
	return trimmed
}
...
function truncateWorkerOutput(worker: { stdout: string; stderr: string; exitCode: number }) {
	return {
		...worker,
