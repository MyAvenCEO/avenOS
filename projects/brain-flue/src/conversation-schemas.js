...
const INTENT_STATUS_VALUES = ['active', 'waiting_for_user', 'completed', 'failed'];
...
export function normalizeFlueResponseData(input) {
    if (input && typeof input === 'object' && 'data' in input) {
        return input.data;
    }
    return input;
}
export function validateDispatcherDecision(input, state) {
    const parsed = flueDispatcherOutputSchema.safeParse(input);
    if (!parsed.success) {
        throw new FlueBrainValidationError(`Invalid dispatcher decision: ${parsed.error.issues.map((issue) => issue.message).join('; ')}`);
    }
...
export function validateIntentDecision(input, context) {
    const parsed = flueIntentOutputSchema.safeParse(input);
    if (!parsed.success) {
        throw new FlueBrainValidationError(`Invalid intent decision: ${parsed.error.issues.map((issue) => issue.message).join('; ')}`);
    }
