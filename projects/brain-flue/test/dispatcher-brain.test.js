...
import { FlueBrainValidationError, createFlueDispatcherBrain } from '../src/index';
...
test('dispatcher accepts create_intent output', async () => {
    ...
    await expect(brain.route({ state: makeDispatcherState(), envelope: makeEnvelopeRecord(), userInput: { text: 'Plan a trip', attachments: [] } })).resolves.toEqual({ type: 'create_intent', title: 'Trip planning', initialGoal: 'Plan a trip', reason: 'Distinct goal' });
});
test('dispatcher rejects route to unknown intent', async () => {
    ...
    await expect(brain.route({ state: makeDispatcherState(), envelope: makeEnvelopeRecord(), userInput: { text: 'hello', attachments: [] } })).rejects.toThrow(FlueBrainValidationError);
});
test('dispatcher rejects route to completed intent', async () => {
    ...
    await expect(brain.route({ state: makeDispatcherState('completed'), envelope: makeEnvelopeRecord(), userInput: { text: 'hello', attachments: [] } })).rejects.toThrow('may not route to completed or failed intents');
});
...
function makeDispatcherState(status = 'active') {
    return {
        activeIntents: {
            'intent-1': {
                intentId: 'intent-1',
