...
import { FlueBrainModelError, createFlueSkillSupervisorBrain } from '../src/index';
const skill = {
    id: 'memory',
...
test('supervisor accepts flue responses wrapped in data', async () => {
    ...
    await expect(brain.decide({ skill, actorState: { skillId: 'memory', workers: {} }, envelope: makeEnvelopeRecord() })).resolves.toMatchObject({ state: { skillId: 'memory', workers: {} } });
});
