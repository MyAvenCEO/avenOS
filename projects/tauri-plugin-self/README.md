...
- **iOS** — Secure Enclave + Face ID/Touch ID via the same Swift bridge as macOS (`swift-lib`; `SwiftLinker::with_ios` in `build.rs`).
- **Linux / Windows / Android** — not implemented for production; Rust uses `dev_insecure` stubs in debug or returns explicit errors outside dev mode.
