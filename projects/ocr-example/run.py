...
def _gemini_api_key() -> str:
    ...
    if not k:
        raise HeadlessError(
            "GEMINI_API_KEY (or GOOGLE_API_KEY) is not set — export it or add it to the repo root .env (see ../../.env from this package)."
        )
    return k
...
def _load_standard_env_files() -> None:
    """Load ``../../.env`` (repo root) then optional ``../.env`` (parent dir) and ``./.env``.

    Later files override earlier for keys they define (so package-local wins over repo root).
    """
    repo_root = PACKAGE_DIR.parent.parent
    _load_dotenv(repo_root / ".env")
    if os.environ.get("HEADLESS_DOTENV_PARENT", "").strip().lower() in {"1", "true", "yes"}:
        _load_dotenv(PACKAGE_DIR.parent / ".env", override=True)
    _load_dotenv(PACKAGE_DIR / ".env", override=True)
