...
Working directory should be **`projects/ocr-example/`** (this folder—the one that contains **`run.py`** and **`requirements.txt`**).
...
**IDE:** Point **Python: default interpreter** at **`projects/ocr-example/.venv/bin/python`** after you create that venv.
...
Put **`GEMINI_API_KEY`** or **`GOOGLE_API_KEY`** in the **AvenOS repo root** **`.env`** (two levels up from this folder: **`../../.env`**). That file is loaded first (keys already in your shell win over the repo file); an optional **`projects/ocr-example/.env`** overrides any earlier value for keys it defines. Format: **`KEY=value`** per line; lines starting with **`#`** are comments. **`extract_document()`** loads the same files on each call.
...
Set **`HEADLESS_DOTENV_PARENT=1`** to also load **`.env`** from **`projects/`** (between repo root and package-local; keys there override repo root, and package **`.env`** still wins last).
## Run (CLI)

From **`projects/ocr-example/`**:
...
From **`projects/ocr-example/`** (this directory on **`PYTHONPATH`**, or as cwd):
