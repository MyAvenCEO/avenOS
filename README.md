...
avenOS keeps the work around a document or request in one place. Drop a file into the
desktop app, ask about it in text or by voice, follow what processes it, and inspect
the artifacts produced along the way. The work lives as an Intent with its source
material, activity, results, and the skills or Actors involved, so a conversation
about a document does not become a separate history from the document itself.
...
The application is a work in progress. The complete system runs locally and the
shared `next` deployment is supported. An independent production installation is not
supported yet. Persistent Intent and Actor services are present, while document
processing still runs in the client process rather than in the generic server-side
runner.
