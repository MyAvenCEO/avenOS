...
test('source workflows cannot acquire installation authority', () => {
	for (const file of readdirSync('.github/workflows')) {
		...
		expect(text).not.toMatch(
			/HETZNER_COMPUTE_TOKEN|BACKUP_RESTIC_PASSWORD|PULUMI_CONFIG_PASSPHRASE|platform-deploy|platform-infrastructure|repository_dispatch|workflow run/
		)
	}
...
test('successful source verification publishes a durable attested manifest', () => {
	const workflow = Bun.YAML.parse(
		readFileSync('.github/workflows/platform-release.yml', 'utf8')
	) as any
