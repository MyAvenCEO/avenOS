#!/usr/bin/env bun
/**
 * Deploy / (re)configure the aven-server relay as a Sprites Service — driven
 ...
 *
 * Prerequisite: the aven-server binary must already be built on the Sprite at
 * AVEN_SERVER_BIN (see docs/deploy/aven-server-sprite runbook). This script only
...
const BIN =
	...
	'/home/sprite/aven-build/target/rust/release/aven-server'
...
console.log(`Deploying aven-server → Sprite "${SPRITE}" (binary: ${BIN})`)
...
const probe = onSprite(['/.sprite/bin/sprite-env', 'services', 'get', 'aven-server'])
...
if (exists && !process.env.WIPE) {
	...
	onSprite(['/.sprite/bin/sprite-env', 'services', 'restart', 'aven-server'])
	...
	onSprite(['/.sprite/bin/sprite-env', 'services', 'stop', 'aven-server'])
	onSprite([
		'sh',
		'-c',
		'for i in $(seq 1 30); do pgrep -f release/aven-server >/dev/null || break; sleep 0.3; done'
	])
	onSprite(['/.sprite/bin/sprite-env', 'services', 'delete', 'aven-server'])
...
const get = onSprite(['/.sprite/bin/sprite-env', 'services', 'get', 'aven-server'])
let status = 'unknown'
try {
...
if (status !== 'running') {
	console.error(`❌ aven-server is not running (status: ${status}).`)
	console.error(`   Logs: sprite exec -s ${SPRITE} -- tail -40 /.sprite/logs/services/aven-server.log`)
	process.exit(1)
}
...
if (!did) {
	const log = onSprite([
		'sh',
		'-c',
		"sed -E 's/\\x1b\\[[0-9;]*m//g' /.sprite/logs/services/aven-server.log | grep -oE 'did:key:[A-Za-z0-9]+' | tail -1"
	])
	did = (log.stdout ?? '').trim() || undefined
}
const healed = /store unreadable . resetting/.test(boot)
console.log(
	`✅ aven-server Service running${healed ? ' (self-healed a corrupt cache; re-pulling from peers)' : ''}. Relay DID: ${did ?? '(see logs)'}`
)
console.log(
	`   Logs:   sprite exec -s ${SPRITE} -- tail -f /.sprite/logs/services/aven-server.log\n` +
