...
function main(): void {
	...
	if (buildNumber) {
		console.log(`[release-app] build number = ${buildNumber} (positional arg, applied to all selected targets)`)
	} else if (process.env.AVEN_BUILD_NUMBER?.trim()) {
		console.log(`[release-app] build number = ${process.env.AVEN_BUILD_NUMBER.trim()} (AVEN_BUILD_NUMBER)`)
	} else if (!skipBuild) {
		console.warn(
	...
	if (skipBuild) console.log('[release-app] --no-build → skipping build, uploading newest dist artifact')
	...
	for (const target of targets) {
		...
		if (skipBuild) {
			...
			if (buildStatus !== 0) {
				...
				console.error(`[release-app] ${target} build failed after ${buildSecs} (exit ${buildStatus})`)
				process.exit(buildStatus)
			}
			console.log(`[release-app] ${target} build done in ${buildSecs}`)
			artifactPath = newestArtifact(spec)
			if (!artifactPath) {
				console.error(`[release-app] build succeeded but no ${spec.artifactExt} found in ${spec.distDir}`)
				process.exit(1)
			}
		}
		...
		if (!skipUpload) {
			...
			if (status !== 0) {
				results.push({ target, build: buildLabel, buildSecs, uploadSecs, uploaded: false, ok: false })
	...
	for (const r of results) {
		...
		console.log(`  ${r.ok ? '✓' : '✗'} ${r.target.padEnd(4)} build=${String(r.build).padEnd(7)} ${flag.padEnd(13)} ${parts.join(' + ') || '—'}`)
	}
